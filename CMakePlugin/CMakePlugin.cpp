    ...
    , m_panel(NULL)
{
    ...
    m_configuration.reset(new CMakeConfiguration(clStandardPaths::Get().
        GetUserDataDir() + wxFileName::GetPathSeparator() + "config/cmake.ini")
    );

    // Create cmake application
    m_cmake.reset(new CMake(this, m_configuration->GetProgramPath()));
...
CMakePlugin::OnHelp(wxCommandEvent& event)
{
    ...
    CMakeHelpDialog(EventNotifier::Get()->TopFrame(), m_cmake.get()).ShowModal();
}
