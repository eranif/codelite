...
void ASFormatter::buildLanguageVectors()
{
    ...
    if(operators->empty()) ASResource::buildOperators(operators, getFileType());
    if(assignmentOperators->empty()) ASResource::buildAssignmentOperators(assignmentOperators);
    if(castOperators->empty()) ASResource::buildCastOperators(castOperators);
}
...
void ASFormatter::fixOptionVariableConflicts()
{
    if(formattingStyle == STYLE_ALLMAN) {
        ...
        if(shouldAddBrackets) shouldAddOneLineBrackets = true;
    ...
    if(!getTabLength()) setDefaultTabLength();
    // add-one-line-brackets implies keep-one-line-blocks
    if(shouldAddOneLineBrackets) setBreakOneLineBlocksMode(false);
}
...
string ASFormatter::nextLine()
{
    ...
    while(!isLineReady) {
        ...
        } else // stuff to do when reading a new character...
        {
            ...
            if(isInVirginLine && currentChar == '{' && currentLineBeginsWithBracket // lineBeginsWith('{')
               ...
               previousCommandChar == ' ')
                previousCommandChar = '{';
            if(isInHorstmannRunIn) isInLineBreak = false;
            if(!isWhiteSpace(currentChar)) isInHorstmannRunIn = false;
        ...
        if(shouldBreakLineAtNextChar) {
            if(isWhiteSpace(currentChar) && !lineIsEmpty) continue;
            isInLineBreak = true;
            shouldBreakLineAtNextChar = false;
        }

        if(isInExecSQL && !passedSemicolon) {
            if(currentChar == ';') passedSemicolon = true;
            appendCurrentChar();
            continue;
        }
        ...
        // treat these preprocessor statements as a line comment
            ...
            if(preproc.compare(0, 6, "region") == 0 || preproc.compare(0, 9, "endregion") == 0 ||
               preproc.compare(0, 5, "error") == 0 || preproc.compare(0, 7, "warning") == 0) {
        ...
        if(isInTemplate && shouldCloseTemplates) {
            if(previousCommandChar == '<' && isWhiteSpace(currentChar)) continue;
            if(isWhiteSpace(currentChar) && peekNextChar() == '>') continue;
        }
        ...
        if(isImmediatelyPostHeader) {
            ...
            if(currentChar != '{' && shouldAddBrackets) {
                ...
                if(bracketsAdded && !shouldAddOneLineBrackets) {
                    ...
                    if((int)firstText == charNum) breakCurrentOneLineBlock = true;
                }
            }
            ...
            if(shouldBreakOneLineStatements && isOkToBreakBlock(bracketTypeStack->back())) {
                // if may break 'else if()'s, then simply break the line
                if(shouldBreakElseIfs) isInLineBreak = true;
            }
        ...
        if(currentChar == '(' || currentChar == '[' || (isInTemplate && currentChar == '<')) {
            parenStack->back()++;
            if(currentChar == '[') isInBlParen = true;
            ...
            if(currentChar == ']') isInBlParen = false;
            if(currentChar == ')') {
                foundCastOperator = false;
                if(parenStack->back() == 0) endOfAsmReached = true;
            }
        }
        ...
        if(currentChar == '{' || currentChar == '}') {
            ...
            bool isOpeningArrayBracket =
                (isBracketType(bracketType, ARRAY_TYPE) && bracketTypeStack->size() >= 2 &&
        ...
        if((((previousCommandChar == '{' && isPreviousBracketBlockRelated) ||
             ((previousCommandChar == '}' && !isImmediatelyPostEmptyBlock && isPreviousBracketBlockRelated &&
           ...
           (previousCommandChar == '{' // added 9/30/2010
            ...
            !isBracketType(bracketTypeStack->back(), SINGLE_LINE_TYPE) && isNonInStatementArray)) {
            isCharImmediatelyPostOpenBlock = (previousCommandChar == '{');
            isCharImmediatelyPostCloseBlock = (previousCommandChar == '}');

            if(isCharImmediatelyPostOpenBlock && !isCharImmediatelyPostComment && !isCharImmediatelyPostLineComment) {
                previousCommandChar = ' ';
                ...
                if(bracketFormatMode == NONE_MODE) {
                    if(shouldBreakOneLineBlocks && isBracketType(bracketTypeStack->back(), SINGLE_LINE_TYPE))
                        ...
                        breakLine();
                } else if(bracketFormatMode == RUN_IN_MODE && currentChar != '#')
                    formatRunIn();
                else
                    isInLineBreak = true;
            } else if(isCharImmediatelyPostCloseBlock && shouldBreakOneLineStatements &&
        ...
        if(isPotentialHeader && !isInTemplate) {
            ...
            if(newHeader != NULL) {
                ...
                if(foundClosingHeader && previousNonWSChar == '}') {
                    if(isOkToBreakBlock(bracketTypeStack->back())) isLineBreakBeforeClosingHeader();
                ...
                if(shouldPadHeader && (!isNonParenHeader || (currentHeader == &AS_CASE && peekNextChar() == '(') ||
                ...
                if(currentHeader != &AS_CASE && !(foundClosingHeader && currentHeader == &AS_WHILE)) {
                    isInHeader = true;
                ...
                if(shouldBreakBlocks && isOkToBreakBlock(bracketTypeStack->back())) {
                    if(previousHeader == NULL && !foundClosingHeader && !isCharImmediatelyPostOpenBlock &&
                       !isImmediatelyPostCommentOnly) {
                        isPrependPostBlockEmptyLineRequested = true;
                    }

                    if(currentHeader == &AS_ELSE || currentHeader == &AS_CATCH || currentHeader == &AS_FINALLY ||
                       foundClosingHeader) {
                        isPrependPostBlockEmptyLineRequested = false;
                    }

                    if(shouldBreakClosingHeaderBlocks && isCharImmediatelyPostCloseBlock &&
                       !isImmediatelyPostCommentOnly && currentHeader != &AS_WHILE) // closing do-while block
                    {
                        isPrependPostBlockEmptyLineRequested = true;
                    }
                }

                if(currentHeader == &AS_CASE || currentHeader == &AS_DEFAULT) isInCase = true;

                continue;
            } else if((newHeader = findHeader(preDefinitionHeaders)) != NULL && parenStack->back() == 0) {
                if(newHeader == &AS_NAMESPACE) foundNamespaceHeader = true;
                if(newHeader == &AS_CLASS) foundClassHeader = true;
                if(newHeader == &AS_STRUCT) foundStructHeader = true;
                if(newHeader == &AS_INTERFACE) foundInterfaceHeader = true;
        ...
        if(previousNonWSChar == '}' || currentChar == ';') {
            if(currentChar == ';') {
                if(((shouldBreakOneLineStatements || isBracketType(bracketTypeStack->back(), SINGLE_LINE_TYPE)) &&
                    isOkToBreakBlock(bracketTypeStack->back())) &&
                ...
                if(shouldBreakBlocks && currentHeader != NULL && currentHeader != &AS_CASE &&
                   currentHeader != &AS_DEFAULT && parenStack->back() == 0) {
            ...
            if(currentChar != ';' || (needHeaderOpeningBracket && parenStack->back() == 0)) currentHeader = NULL;
        ...
        if(currentChar == ':') {
            ...
            {
                isInCase = false;
                if(shouldBreakOneLineStatements) passedColon = true;
            } else if(isCStyle() // for C/C++ only
                      &&
                      shouldBreakOneLineStatements && !foundQuestionMark // not in a ... ? ... : ... sequence
                      &&
                      !foundPreDefinitionHeader // not in a definition block (e.g. class foo : public bar
                      &&
                      previousCommandChar != ')' // not immediately after closing paren of a method header, e.g.
                                                 // ASFormatter::ASFormatter(...) : ASBeautifier(...)
                      &&
        ...
        if(currentChar == '?') foundQuestionMark = true;

        if(isPotentialHeader && !isInTemplate) {
            if(findKeyword(currentLine, charNum, AS_NEW)) isInPotentialCalculation = false;
            ...
            if(isCStyle() && findKeyword(currentLine, charNum, AS_THROW) && previousCommandChar != ')' &&
               !foundPreCommandHeader) // 'const' throw()
                isImmediatelyPostThrow = true;

            if(findKeyword(currentLine, charNum, AS_OPERATOR)) isImmediatelyPostOperator = true;

            if(isCStyle() && findKeyword(currentLine, charNum, AS_ENUM)) isInEnum = true;

            if(isCStyle() && findKeyword(currentLine, charNum, AS_EXTERN)) isInExtern = true;

            if(isCStyle() && isExecSQL(currentLine, charNum)) isInExecSQL = true;

            if(isCStyle()) {
                if(findKeyword(currentLine, charNum, AS_ASM) || findKeyword(currentLine, charNum, AS__ASM__)) {
                    isInAsm = true;
            ...
            if(isJavaStyle() && (findKeyword(currentLine, charNum, AS_STATIC) && isNextCharOpeningBracket(charNum + 6)))
                isJavaStaticConstructor = true;

            if(isSharpStyle() &&
               (findKeyword(currentLine, charNum, AS_DELEGATE) || findKeyword(currentLine, charNum, AS_UNCHECKED)))
            ...
            if(shouldPadOperators && (name == "and" || name == "or")) {
                appendSpacePad();
                appendSequence(name);
                goForward(name.length() - 1);
                if(!isBeforeAnyComment() && !(currentLine.compare(charNum + 1, 1, ";") == 0) &&
        ...
        if(isPotentialOperator) {
            ...
            if(newHeader != NULL) {
                ...
                if(isInTemplate && (newHeader == &AS_GR_GR || newHeader == &AS_GR_GR_GR)) newHeader = &AS_GR;
        ...
        if(!isJavaStyle() && (newHeader == &AS_MULT || newHeader == &AS_BIT_AND || newHeader == &AS_BIT_XOR) &&
           isPointerOrReference() && !isDereferenceOrAddressOf()) {
        ...
        if(currentChar == ';' || (currentChar == ',' && shouldPadOperators)) {
            char nextChar = ' ';
            if(charNum + 1 < (int)currentLine.length()) nextChar = currentLine[charNum + 1];
        ...
        if(currentChar == '(' && shouldPadHeader && (isCharImmediatelyPostReturn || isCharImmediatelyPostThrow))
            appendSpacePad();
    ...
    } else // format the current formatted line
    {
        ...
        if(!lineCommentNoBeautify) enhancer->enhance(beautifiedLine, isInPreprocessorBeautify, isInBeautifySQL);
        horstmannIndentChars = 0;
        lineCommentNoBeautify = lineCommentNoIndent;
        lineCommentNoIndent = false;
 ...
 */
bool ASFormatter::isBracketType(BracketType a, BracketType b) const { return ((a & b) == b); }
 ...
 */
void ASFormatter::setFormattingStyle(FormatStyle style) { formattingStyle = style; }
 ...
 */
void ASFormatter::setAddBracketsMode(bool state) { shouldAddBrackets = state; }
 ...
 * @param mode         the bracket formatting mode.
 */
void ASFormatter::setBracketFormatMode(BracketMode mode) { bracketFormatMode = mode; }
 ...
 */
void ASFormatter::setBreakAfterMode(bool state) { shouldBreakLineAfterLogical = state; }
 ...
 */
void ASFormatter::setBreakClosingHeaderBracketsMode(bool state) { shouldBreakClosingHeaderBrackets = state; }
 ...
 */
void ASFormatter::setBreakElseIfsMode(bool state) { shouldBreakElseIfs = state; }
 ...
 */
void ASFormatter::setMaxCodeLength(int max) { maxCodeLength = max; }
 ...
 */
void ASFormatter::setOperatorPaddingMode(bool state) { shouldPadOperators = state; }
 ...
 * @param state         the padding mode.
 */
void ASFormatter::setParensOutsidePaddingMode(bool state) { shouldPadParensOutside = state; }
 ...
 * @param state         the padding mode.
 */
void ASFormatter::setParensInsidePaddingMode(bool state) { shouldPadParensInside = state; }
 ...
 * @param state         the padding mode.
 */
void ASFormatter::setParensFirstPaddingMode(bool state) { shouldPadFirstParen = state; }
 ...
 * @param state         the padding mode.
 */
void ASFormatter::setParensHeaderPaddingMode(bool state) { shouldPadHeader = state; }
 ...
 * @param state         the padding mode.
 */
void ASFormatter::setParensUnPaddingMode(bool state) { shouldUnPadParens = state; }
 ...
 */
void ASFormatter::setAttachClosingBracket(bool state) { shouldAttachClosingBracket = state; }
 ...
 */
void ASFormatter::setBreakOneLineBlocksMode(bool state) { shouldBreakOneLineBlocks = state; }

void ASFormatter::setCloseTemplatesMode(bool state) { shouldCloseTemplates = state; }
 ...
 * @param state        true = break, false = don't break.
 */
void ASFormatter::setSingleStatementsMode(bool state) { shouldBreakOneLineStatements = state; }
 ...
 */
void ASFormatter::setTabSpaceConversionMode(bool state) { shouldConvertTabs = state; }
 ...
 */
void ASFormatter::setIndentCol1CommentsMode(bool state) { shouldIndentCol1Comments = state; }
 ...
 */
void ASFormatter::setLineEndFormat(LineEndFormat fmt) { lineEnd = fmt; }
 ...
 * @param state        true = convert, false = don't convert.
 */
void ASFormatter::setBreakClosingHeaderBlocksMode(bool state) { shouldBreakClosingHeaderBlocks = state; }
 ...
 */
void ASFormatter::setDeleteEmptyLinesMode(bool state) { shouldDeleteEmptyLines = state; }
 ...
 */
void ASFormatter::setPointerAlignment(PointerAlign alignment) { pointerAlignment = alignment; }

void ASFormatter::setReferenceAlignment(ReferenceAlign alignment) { referenceAlignment = alignment; }
...
bool ASFormatter::isBeforeComment() const
{
    ...
    if(peekNum == string::npos) return foundComment;
...
bool ASFormatter::isBeforeAnyComment() const
{
    ...
    foundComment = (currentLine.compare(peekNum, 2, "/*") == 0 || currentLine.compare(peekNum, 2, "//") == 0);
    ...
    if(peekNum != string::npos) {
        if(currentLine.compare(peekNum, 2, "//") == 0)
            ...
            if(endNum != string::npos) {
                ...
                if(nextChar == string::npos) foundLineEndComment = true;
            }
        }
    }
...
bool ASFormatter::isBeforeMultipleLineEndComments(int startPos) const
{
    ...
    if(peekNum != string::npos) {
        if(currentLine.compare(peekNum, 2, "/*") == 0) {
            ...
            if(endNum != string::npos) {
                size_t nextChar = currentLine.find_first_not_of(" \t", endNum + 2);
                if(nextChar != string::npos && currentLine.compare(nextChar, 2, "//") == 0)
...
bool ASFormatter::getNextChar()
{
    ...
    if(!isWhiteSpace(currentChar)) {
        ...
        if(!isInComment && !isInLineComment && !isInQuote && !isImmediatelyPostComment &&
           !isImmediatelyPostLineComment && !isInPreprocessor && !isSequenceReached("/*") && !isSequenceReached("//"))
            previousCommandChar = currentChar;
    }

    if(charNum + 1 < (int)currentLine.length() && (!isWhiteSpace(peekNextChar()) || isInComment || isInLineComment)) {
        currentChar = currentLine[++charNum];

        if(shouldConvertTabs && currentChar == '\t') convertTabToSpaces();
...
bool ASFormatter::getNextLine(bool emptyLineWasDeleted /*false*/)
{
    if(sourceIterator->hasMoreLines()) {
        ...
        inLineNumber++;
        if(endOfAsmReached) endOfAsmReached = isInAsm = false;
        shouldKeepLineUnbroken = false;
        isInCommentStartLine = false;
        isInCase = false;
        isInAsmOneLine = false;
        ...
        if(currentLine.length() == 0) currentLine = string(" "); // a null is inserted if this is not done
        ...
        if(currentHeader == &AS_CASE && isInLineBreak && !isImmediatelyPostLineComment) {
            // check for split line
            if((formattedLine.length() >= 4 && formattedLine.substr(formattedLine.length() - 4, 4) == "case") ||
               (formattedLine[formattedLine.length() - 1] == '\'' && findNextChar(currentLine, ':') != string::npos)) {
                isInLineBreak = false;
                isInCase = true;
                if(formattedLine.substr(formattedLine.length() - 4, 4) == "case") appendSpacePad();
            }
        }
        // END OF FIX
        ...
        if(!isInComment && (previousNonWSChar != '\\' || isEmptyLine(currentLine))) isInPreprocessor = false;

        if(passedSemicolon) isInExecSQL = false;
        initNewLine();

        currentChar = currentLine[charNum];
        if(isInHorstmannRunIn && previousNonWSChar == '{' && !isInComment) isInLineBreak = false;
        isInHorstmannRunIn = false;
...
void ASFormatter::initNewLine()
{
    ...
    // don't trim these
    if(isInQuoteContinuation || (isInPreprocessor && !getPreprocessorIndent())) return;
    ...
    if(isInComment) {
        if(noTrimCommentContinuation) leadingSpaces = tabIncrementIn = 0;
        trimContinuationLine();
        return;
    }
    ...
    if(isSequenceReached("/*")) {
        ...
        if(firstText != string::npos) {
            if(currentLine.compare(firstText, 2, "//") == 0)
                lineIsLineCommentOnly = true;
            else if(currentLine.compare(firstText, 2, "/*") == 0 || isExecSQL(currentLine, firstText)) {
                // get the extra adjustment
                size_t j;
                for(j = charNum + 1; isWhiteSpace(currentLine[j]) && j < firstText; j++) {
                    if(currentLine[j] == '\t') tabIncrementIn += tabSize - 1 - ((tabIncrementIn + j) % tabSize);
                }
                leadingSpaces = j + tabIncrementIn;
                if(currentLine.compare(firstText, 2, "/*") == 0) doesLineStartComment = true;
            }
        }
    ...
    if(isInPreprocessor) {
        if(!doesLineStartComment) leadingSpaces = 0;
        charNum = 0;
    }
}
...
{
    if(canBreakLine && isInLineBreak) breakLine();
...
BracketType ASFormatter::getBracketType()
{
    ...
    if((previousNonWSChar == '=' || isBracketType(bracketTypeStack->back(), ARRAY_TYPE)) && previousCommandChar != ')')
        ...
        bool isCommandType =
            (foundPreCommandHeader || (currentHeader != NULL && isNonParenHeader) || (previousCommandChar == ')') ||
             (previousCommandChar == ':' && !foundQuestionMark) || (previousCommandChar == ';') ||
             ((previousCommandChar == '{' || previousCommandChar == '}') && isPreviousBracketBlockRelated) ||
             isJavaStaticConstructor || isSharpDelegate);
    ...
    if(foundOneLineBlock == 2 && returnVal == COMMAND_TYPE) returnVal = ARRAY_TYPE;
 ...
 */
bool ASFormatter::isEmptyLine(const string& line) const { return line.find_first_not_of(" \t") == string::npos; }
...
bool ASFormatter::isPointerOrReference() const
{
    ...
    if(isJavaStyle()) return false;

    if((currentChar == '&' && previousChar == '&') || isCharImmediatelyPostOperator) return false;

    if(previousNonWSChar == '=' || previousNonWSChar == '(' || previousNonWSChar == '[' ||
       isCharImmediatelyPostReturn || isInTemplate || isCharImmediatelyPostTemplate || currentHeader == &AS_CATCH)
        return true;

    // get the last legal word (may be a number)
    string lastWord = getPreviousWord(currentLine, charNum);
    if(lastWord.empty()) lastWord = " ";
    char nextChar = peekNextChar();

    // check for preceding or following numeric values
    if(isDigit(lastWord[0]) || isDigit(nextChar) || nextChar == '!') return false;

    if(isBracketType(bracketTypeStack->back(), ARRAY_TYPE) && isLegalNameChar(lastWord[0]) &&
       isLegalNameChar(nextChar) && previousNonWSChar != ')') {
        if(isArrayOperator()) return false;
    }

    // checks on operators in parens
    if(parenStack->back() > 0 && isLegalNameChar(lastWord[0]) && isLegalNameChar(nextChar)) {
        // if followed by an assignment it is a pointer or reference
        const string* followingOperator = getFollowingOperator();
        if(followingOperator && followingOperator != &AS_MULT && followingOperator != &AS_BIT_AND) {
    ...
    if(parenStack->back() > 0 && nextChar == '(' && previousNonWSChar != ',' && previousNonWSChar != '(' &&
       previousNonWSChar != '!' && previousNonWSChar != '&' && previousNonWSChar != '*' && previousNonWSChar != '|')
        return false;

    if(nextChar == '-' || nextChar == '+') {
        size_t nextNum = currentLine.find_first_not_of(" \t", charNum + 1);
        if(nextNum != string::npos) {
            if(currentLine.compare(nextNum, 2, "++") != 0 && currentLine.compare(nextNum, 2, "--") != 0) return false;
        }
    }

    bool isPR =
        (!isInPotentialCalculation || isBracketType(bracketTypeStack->back(), DEFINITION_TYPE) ||
         (!isLegalNameChar(previousNonWSChar) && !(previousNonWSChar == ')' && nextChar == '(') &&
          !(previousNonWSChar == ')' && currentChar == '*' && !isImmediatelyPostCast()) && previousNonWSChar != ']'));

    if(!isPR) {
        isPR |= (!isWhiteSpace(nextChar) && nextChar != '-' && nextChar != '(' && nextChar != '[' &&
...
bool ASFormatter::isDereferenceOrAddressOf() const
{
    ...
    if(isCharImmediatelyPostTemplate) return false;

    if(previousNonWSChar == '=' || previousNonWSChar == ',' || previousNonWSChar == '.' || previousNonWSChar == '{' ||
       previousNonWSChar == '>' || previousNonWSChar == '<' || isCharImmediatelyPostLineComment ||
       isCharImmediatelyPostComment || isCharImmediatelyPostReturn)
        return true;

    // check for **
    if(currentChar == '*' && (int)currentLine.length() > charNum && currentLine[charNum + 1] == '*') {
        if(previousNonWSChar == '(') return true;
        if((int)currentLine.length() < charNum + 2) return true;
        return false;
    }

    // check first char on the line
    if(charNum == (int)currentLine.find_first_not_of(" \t")) return true;

    char nextChar = peekNextChar();
    if(nextChar == ')' || nextChar == '>' || nextChar == ',') return false;

    // check for reference to a pointer *& (cannot have &*)
    if((currentChar == '*' && nextChar == '&') || (previousNonWSChar == '*' && currentChar == '&')) return false;

    if(!isBracketType(bracketTypeStack->back(), COMMAND_TYPE) && parenStack->back() == 0) return false;

    string lastWord = getPreviousWord(currentLine, charNum);
    if(lastWord == "else" || lastWord == "delete") return true;

    bool isDA =
        (!(isLegalNameChar(previousNonWSChar) || previousNonWSChar == '>') ||
         (!isLegalNameChar(nextChar) && nextChar != '/') ||
         (ispunct((unsigned char)previousNonWSChar) && previousNonWSChar != '.') || isCharImmediatelyPostReturn);
...
bool ASFormatter::isPointerOrReferenceCentered() const
{
    ...
    if(peekNextChar() == ' ') return false;

    // check space before
    if(prNum < 1 || currentLine[prNum - 1] != ' ') return false;

    // check no space before that
    if(prNum < 2 || currentLine[prNum - 2] == ' ') return false;

    // check for **
    if(prNum + 1 < lineLength && currentLine[prNum + 1] == '*') prNum++;

    // check space after
    if(prNum + 1 <= lineLength && currentLine[prNum + 1] != ' ') return false;

    // check no space after that
    if(prNum + 2 < lineLength && currentLine[prNum + 2] == ' ') return false;
...
bool ASFormatter::isUnaryOperator() const
{
    ...
    return ((isCharImmediatelyPostReturn || !isLegalNameChar(previousCommandChar)) && previousCommandChar != '.' &&
...
bool ASFormatter::isNonInStatementArrayBracket() const
{
    ...
    if(currentLineBeginsWithBracket && charNum == (int)currentLineFirstBracketNum && nextChar != '}') returnVal = true;
    // if an opening bracket ends the line there will be no inStatement indent
    if(isWhiteSpace(nextChar) || isBeforeAnyLineEndComment(charNum) || nextChar == '{') returnVal = true;

    // Java "new Type [] {...}" IS an inStatement indent
    if(isJavaStyle() && previousNonWSChar == ']') returnVal = false;
    ...
    for(int i = startChar + 1; i < lineLength; ++i) {
        ...
        if(bracketCount == 0) {
            ...
            if(parenStack->back() == 0 && prevCh != '}') {
                ...
                if(peekNum != string::npos && line[peekNum] == ',') return 2;
            }
            return 1;
        }
        if(!isWhiteSpace(ch)) prevCh = ch;
    }
...
bool ASFormatter::isNextWordSharpNonParenHeader(int startChar) const
{
    ...
    if(nextText[0] == '[') return true;
    if(!isCharPotentialHeader(nextText, 0)) return false;
    if(findKeyword(nextText, 0, AS_GET) || findKeyword(nextText, 0, AS_SET) || findKeyword(nextText, 0, AS_ADD) ||
...
bool ASFormatter::isNextCharOpeningBracket(int startChar) const
{
    bool retVal = false;
    string nextText = peekNextText(currentLine.substr(startChar));
    if(nextText.compare(0, 1, "{") == 0) retVal = true;
    return retVal;
}
 ...
 */
string
ASFormatter::peekNextText(const string& firstLine, bool endOnEmptyLine /*false*/, bool shouldReset /*false*/) const
{
    ...
    while(sourceIterator->hasMoreLines()) {
        ...
        if(firstChar == string::npos) {
            if(endOnEmptyLine && !isInComment_) break;
            continue;
        }
        ...
        if(nextLine_.compare(firstChar, 2, "//") == 0) continue;
...
void ASFormatter::adjustComments(void)
{
    ...
    assert(currentLine.compare(charNum, 2, "//") == 0 || currentLine.compare(charNum, 2, "/*") == 0);
    ...
    if(currentLine.compare(charNum, 2, "/*") == 0) {
        ...
        if(endNum == string::npos) return;
        if(currentLine.find_first_not_of(" \t", endNum + 2) != string::npos) return;
    }

    size_t len = formattedLine.length();
    // don't adjust a tab
    if(formattedLine[len - 1] == '\t') return;
    ...
    // if cannot be done put the comment one space after the last text
        ...
        if(lastText != string::npos && lastText < len - adjust - 1)
...
void ASFormatter::padOperators(const string* newOperator)
{
    ...
    bool shouldPad =
        (newOperator != &AS_COLON_COLON && newOperator != &AS_PLUS_PLUS && newOperator != &AS_MINUS_MINUS &&
         newOperator != &AS_NOT && newOperator != &AS_BIT_NOT && newOperator != &AS_ARROW &&
         ...
         !((newOperator == &AS_PLUS || newOperator == &AS_MINUS) // check for unary plus or minus
           ...
           (previousNonWSChar == '(' || previousNonWSChar == '[' || previousNonWSChar == '=' ||
            previousNonWSChar == ',')) &&
         !(newOperator == &AS_PLUS && isInExponent()) && !isCharImmediatelyPostOperator &&
         !((newOperator == &AS_MULT || newOperator == &AS_BIT_AND) && isPointerOrReference()) &&
         !(newOperator == &AS_MULT && (previousNonWSChar == '.' || previousNonWSChar == '>')) // check for ->
         &&
         !((isInTemplate || isImmediatelyPostTemplate) && (newOperator == &AS_LS || newOperator == &AS_GR)) &&
         !(newOperator == &AS_GCC_MIN_ASSIGN && ASBase::peekNextChar(currentLine, charNum + 1) == '>') &&
         !(newOperator == &AS_GR && previousNonWSChar == '?') &&
         !(newOperator == &AS_QUESTION // check for Java wildcard
           &&
           (previousNonWSChar == '<' || ASBase::peekNextChar(currentLine, charNum) == '>' ||
            ASBase::peekNextChar(currentLine, charNum) == '.')) &&
         !isInCase && !isInAsm && !isInAsmOneLine && !isInAsmBlock);

    // pad before operator
    if(shouldPad && !(newOperator == &AS_COLON && (!foundQuestionMark && !isInEnum) && currentHeader != &AS_FOR) &&
    ...
    if(shouldPad && !isBeforeAnyComment() && !(newOperator == &AS_PLUS && isUnaryOperator()) &&
       !(newOperator == &AS_MINUS && isUnaryOperator()) && !(currentLine.compare(charNum + 1, 1, ";") == 0) &&
       !(currentLine.compare(charNum + 1, 2, "::") == 0) &&
       !(newOperator == &AS_QUESTION && isSharpStyle() // check for C# nullable type (e.g. int?)
         &&
         peekNextChar() == '['))
...
void ASFormatter::formatPointerOrReference(void)
{
    ...
    // check for cast
    char peekedChar = peekNextChar();
    if(currentChar == '*' && (int)currentLine.length() > charNum + 1 && currentLine[charNum + 1] == '*') {
    ...
    if(charNum > 0 && !isWhiteSpace(currentLine[charNum - 1]) && formattedLine.length() > 0 &&
    ...
    if(itemAlignment == PTR_ALIGN_TYPE) {
        ...
        if(prevCh == string::npos) prevCh = 0;
        ...
        if(charNum < (int)currentLine.length() - 1 && !isWhiteSpace(currentLine[charNum + 1]) &&
           currentLine[charNum + 1] != ')')
            appendSpacePad();
        // if old pointer or reference is centered, remove a space
        if(isOldPRCentered && isWhiteSpace(formattedLine[formattedLine.length() - 1])) {
        ...
        // if reference to a pointer check for conflicting alignment
                ...
                (referenceAlignment == REF_ALIGN_TYPE || referenceAlignment == REF_ALIGN_MIDDLE ||
                 referenceAlignment == REF_SAME_AS_PTR)) {
            sequenceToInsert = "*&";
            goForward(1);
        ...
        bool isAfterScopeResolution = previousNonWSChar == ':'; // check for ::
        // if this is not the last thing on the line
        if(!isBeforeAnyComment() && (int)currentLine.find_first_not_of(" \t", charNum + 1) > charNum) {
            // goForward() to convert tabs to spaces, if necessary,
            // and move following characters to preceding characters
            // this may not work every time with tab characters
            for(size_t i = charNum + 1; i < currentLine.length() && isWhiteSpace(currentLine[i]); i++) {
                // if a padded paren follows don't move
                if(shouldPadParensOutside && peekedChar == '(' && !isOldPRCentered) break;
                goForward(1);
                formattedLine.append(1, currentLine[i]);
            }
        }
        ...
        if(startNum != string::npos && isAfterScopeResolution) {
            size_t lastText = formattedLine.find_last_not_of(" \t");
            if(lastText + 1 < formattedLine.length()) formattedLine.erase(lastText + 1);
        }
        // if no space before * then add one
        else if(formattedLine.length() <= startNum + 1 || !isWhiteSpace(formattedLine[startNum + 1])) {
            formattedLine.insert(startNum + 1, 1, ' ');
            spacePadNum++;
        }
        appendSequence(sequenceToInsert, false);
        // if old pointer or reference is centered, remove a space
        if(isOldPRCentered && formattedLine.length() > startNum + 1 && isWhiteSpace(formattedLine[startNum + 1]) &&
           !isBeforeAnyComment()) {
...
void ASFormatter::formatPointerOrReferenceCast(void)
{
    ...
    size_t prevCh = formattedLine.find_last_not_of(" \t");
    if(prevCh == string::npos) prevCh = 0;
    if(prevCh + 1 < formattedLine.length() && isWhiteSpace(formattedLine[prevCh + 1])) {
...
void ASFormatter::padParens(void)
{
    ...
    if(currentChar == '(') {
        ...
        if(shouldUnPadParens) {
            ...
            if(i != string::npos) {
                ...
                if(formattedLine[i] == '{')
                    ...
                    if(shouldPadHeader && prevWord.length() > 0 && isCharPotentialHeader(prevWord, 0))
                    ...
                    else if(prevWord == "bool" || prevWord == "int" || prevWord == "void" || prevWord == "void*" ||
                            (prevWord.length() >= 6 // check end of word for _t
                             &&
                             prevWord.compare(prevWord.length() - 2, 2, "_t") == 0) ||
                            prevWord == "BOOL" || prevWord == "DWORD" || prevWord == "HWND" || prevWord == "INT" ||
                            prevWord == "LPSTR" || prevWord == "VOID" || prevWord == "LPVOID") {
                        prevIsParenHeader = true;
                        // trace
            ...
            if(shouldPadParensOutside || prevIsParenHeader)
                spacesOutsideToDelete--;
                    ...
                    lastChar == '&' // check for &&
                    ||
                    lastChar == ',' || (lastChar == '(' && shouldPadParensInside) ||
                    (lastChar == '>' && !foundCastOperator) || lastChar == '<' || lastChar == '?' || lastChar == ':' ||
        ...
        if(shouldPadFirstParen && previousChar != '(')
            ...
            if(!(currentChar == '(' && peekedCharOutside == ')')) appendSpacePad();
        }
        ...
        if(shouldUnPadParens) {
            ...
            if(j != string::npos) spacesInsideToDelete = j - charNum - 1;
            if(shouldPadParensInside) spacesInsideToDelete--;
            ...
            if(shouldConvertTabs && (int)currentLine.length() > charNum + 1 && currentLine[charNum + 1] == '\t')
                currentLine[charNum + 1] = ' ';
        }
        ...
        if(shouldPadParensInside)
            if(!(currentChar == '(' && peekedCharInside == ')')) appendSpaceAfter();
        // trace
        ...
        if(shouldUnPadParens) {
            size_t i = formattedLine.find_last_not_of(" \t");
            if(i != string::npos) spacesInsideToDelete = formattedLine.length() - 1 - i;
            if(shouldPadParensInside) spacesInsideToDelete--;
            if(spacesInsideToDelete > 0) {
        ...
        char peekedCharOutside = peekNextChar();
        if(shouldPadParensOutside)
            if(peekedCharOutside != ';' && peekedCharOutside != ',' && peekedCharOutside != '.' &&
    ...
    if(breakBracket) {
        if(isBeforeAnyComment() && isOkToBreakBlock(bracketType)) {
            ...
            if(isBeforeAnyLineEndComment(charNum) && !currentLineBeginsWithBracket) // lineBeginsWith('{')
            {
                ...
                if(parenStack->size() > 1) parenStack->pop_back();
        ...
        if(isBeforeComment() && formattedLine.length() > 0 && formattedLine[0] == '{' &&
           isOkToBreakBlock(bracketType) && (bracketFormatMode == BREAK_MODE || bracketFormatMode == LINUX_MODE ||
    ...
    } else // attach bracket
    {
        ...
        if(isCharImmediatelyPostComment || isCharImmediatelyPostLineComment) {
            if(isOkToBreakBlock(bracketType) &&
               ...
               &&
               !isImmediatelyPostPreprocessor && peekNextChar() != '}' // don't attach { }
               &&
        ...
        } else if(previousCommandChar == '{' || previousCommandChar == '}' ||
                  previousCommandChar ==
                      ';') // '}' , ';' chars added for proper handling of '{' immediately after a '}' or ';'
        {
            ...
            if(isEmptyLine(formattedLine))
                appendCurrentChar(); // don't attach
                    ...
                    !(isImmediatelyPostPreprocessor && currentLineBeginsWithBracket)) // lineBeginsWith('{')
            {
                if(peekNextChar() != '}') {
                    appendSpacePad();
                    ...
                    if(isBeforeComment() && !isBeforeMultipleLineEndComments(charNum) &&
                ...
                if(!isInLineBreak) appendSpacePad();
                appendCurrentChar(); // don't attach
            }
        }
    }
...
void ASFormatter::formatClosingBracket(BracketType bracketType)
{
    ...
    if(previousCommandChar == '{') isImmediatelyPostEmptyBlock = true;
    ...
    if(isLegalNameChar(peekNextChar())) appendSpaceAfter();
    ...
    if(shouldBreakBlocks && currentHeader != NULL && parenStack->back() == 0) {
        if(currentHeader == &AS_CASE || currentHeader == &AS_DEFAULT) {
            ...
            if(nextText.substr(0, 5) != "break") isAppendPostBlockEmptyLineRequested = true;
        } else
            isAppendPostBlockEmptyLineRequested = true;
    }
}
...
void ASFormatter::formatArrayBrackets(BracketType bracketType, bool isOpeningArrayBracket)
{
    ...
    if(currentChar == '{') {
        ...
        if(isOpeningArrayBracket) {
            if(bracketFormatMode == ATTACH_MODE || bracketFormatMode == LINUX_MODE ||
               bracketFormatMode == STROUSTRUP_MODE) {
                    ...
                    if(isEmptyLine(formattedLine))
                        ...
                        if(currentLineBeginsWithBracket // lineBeginsWith('{')
                           ...
                           !isBracketType(bracketType, SINGLE_LINE_TYPE)) {
                            ...
                            if(currentLineBeginsWithBracket && (int)currentLineFirstBracketNum == charNum)
                                shouldBreakLineAtNextChar = true;
                        } else {
                            appendSpacePad();
                            appendCurrentChar();
        ...
        } else // not the first opening bracket
        {
            if(bracketFormatMode == RUN_IN_MODE) {
                if(previousNonWSChar == '{' && bracketTypeStack->size() > 2 &&
                   !isBracketType((*bracketTypeStack)[bracketTypeStack->size() - 2], SINGLE_LINE_TYPE))
                    formatArrayRunIn();
            } else if(!isInLineBreak && !isWhiteSpace(peekNextChar()) && previousNonWSChar == '{' &&
        ...
        if(shouldAttachClosingBracket) {
            if(isEmptyLine(formattedLine) // if a blank line preceeds this
               ||
               isImmediatelyPostPreprocessor || isCharImmediatelyPostLineComment || isCharImmediatelyPostComment)
                appendCurrentChar(); // don't attach
            else {
                appendSpacePad();
                appendCurrentChar(false); // attach
            ...
            if(!isBracketType(bracketType, SINGLE_LINE_TYPE) || formattedLine.find('{') == string::npos) breakLine();
            appendCurrentChar();
        }

        // if a declaration follows an enum definition, space pad
        char peekedChar = peekNextChar();
        if(isLegalNameChar(peekedChar) || peekedChar == '[') appendSpaceAfter();
    }
}
...
void ASFormatter::formatRunIn()
{
    ...
    if(!isOkToBreakBlock(bracketTypeStack->back())) return; // true;

    // make sure the line begins with a bracket
    size_t lastText = formattedLine.find_last_not_of(" \t");
    if(lastText == string::npos || formattedLine[lastText] != '{') return; // false;

    // make sure the bracket is broken
    if(formattedLine.find_first_not_of(" \t{") != string::npos) return; // false;

    if(isBracketType(bracketTypeStack->back(), NAMESPACE_TYPE)) return; // false;

    bool extraIndent = false;
    isInLineBreak = true;

    // cannot attach a class modifier without indent-classes
    if(isCStyle() && isCharPotentialHeader(currentLine, charNum) &&
       (isBracketType(bracketTypeStack->back(), CLASS_TYPE) ||
        (isBracketType(bracketTypeStack->back(), STRUCT_TYPE) && isInIndentableStruct))) {
        if(findKeyword(currentLine, charNum, AS_PUBLIC) || findKeyword(currentLine, charNum, AS_PRIVATE) ||
           findKeyword(currentLine, charNum, AS_PROTECTED)) {
            if(!getClassIndent()) return; // false;
        } else if(getClassIndent())
            extraIndent = true;
    }

    // cannot attach a 'case' statement without indent-switches
    if(!getSwitchIndent() && isCharPotentialHeader(currentLine, charNum) &&
       (findKeyword(currentLine, charNum, AS_CASE) || findKeyword(currentLine, charNum, AS_DEFAULT)))
        return; // false;

    // extra indent for switch statements
    if(getSwitchIndent() && !preBracketHeaderStack->empty() && preBracketHeaderStack->back() == &AS_SWITCH &&
       ((isLegalNameChar(currentChar) && !findKeyword(currentLine, charNum, AS_CASE)) || isSequenceReached("//") ||
        isSequenceReached("/*")))
        extraIndent = true;
    ...
    if(getForceTabIndentation() && getIndentLength() != getTabLength()) {
        ...
        if(extraIndent) indent.append(indentLength_, ' ');
...
void ASFormatter::formatArrayRunIn()
{
    ...
    // make sure the bracket is broken
    if(formattedLine.find_first_not_of(" \t{") != string::npos) return;

    size_t lastText = formattedLine.find_last_not_of(" \t");
    if(lastText == string::npos || formattedLine[lastText] != '{') return;
...
{
    if(container != NULL) deleteContainer(container);
    container = value;
}
...
void ASFormatter::convertTabToSpaces()
{
    ...
    if(isInQuote || isInQuoteContinuation) return;
...
bool ASFormatter::isOkToBreakBlock(BracketType bracketType) const
{
    ...
    if(isBracketType(bracketType, ARRAY_TYPE) && isBracketType(bracketType, SINGLE_LINE_TYPE)) return false;
    if(!isBracketType(bracketType, SINGLE_LINE_TYPE) || shouldBreakOneLineBlocks || breakCurrentOneLineBlock)
        return true;
    return false;
}
...
{
    if(isSharpStyle() && peekNextChar() == '(' && (header == &AS_CATCH || header == &AS_DELEGATE)) return true;
    return false;
}
...
void ASFormatter::checkForHeaderFollowingComment(const string& firstLine)
{
    ...
    if(nextText.length() == 0 || !isCharPotentialHeader(nextText, 0)) return;
    ...
    if(isClosingHeader(newHeader)) {
        if(!shouldBreakClosingHeaderBlocks) isPrependPostBlockEmptyLineRequested = false;
    }
...
bool ASFormatter::commentAndHeaderFollows()
{
    ...
    if(!sourceIterator->hasMoreLines()) return false;
    string nextLine_ = sourceIterator->peekNextLine();
    size_t firstChar = nextLine_.find_first_not_of(" \t");
    if(firstChar == string::npos ||
       !(nextLine_.compare(firstChar, 2, "//") == 0 || nextLine_.compare(firstChar, 2, "/*") == 0)) {
    ...
    if(nextText.length() == 0 || !isCharPotentialHeader(nextText, 0)) return false;

    const string* newHeader = ASBeautifier::findHeader(nextText, 0, headers);

    if(newHeader == NULL) return false;
...
bool ASFormatter::isCurrentBracketBroken() const
{
    ...
    if(isBracketType((*bracketTypeStack)[bracketTypeStackEnd], EXTERN_TYPE)) {
        if(currentLineBeginsWithBracket || bracketFormatMode == RUN_IN_MODE) breakBracket = true;
    } else if(bracketFormatMode == NONE_MODE) {
        if(currentLineBeginsWithBracket && (int)currentLineFirstBracketNum == charNum) // lineBeginsWith('{')
            breakBracket = true;
        ...
        if(isBracketType((*bracketTypeStack)[bracketTypeStackEnd], CLASS_TYPE)) {
            if(bracketFormatMode == LINUX_MODE) breakBracket = true;
        }
        ...
        // break the first bracket if a function
        else if(bracketTypeStackEnd == 1 && isBracketType((*bracketTypeStack)[bracketTypeStackEnd], COMMAND_TYPE)) {
            breakBracket = true;
            ...
            if(isBracketType((*bracketTypeStack)[bracketTypeStackEnd - 1], NAMESPACE_TYPE) ||
               isBracketType((*bracketTypeStack)[bracketTypeStackEnd - 1], EXTERN_TYPE)) {
                if(isBracketType((*bracketTypeStack)[bracketTypeStackEnd], COMMAND_TYPE)) breakBracket = true;
            }
...
void ASFormatter::formatCommentBody()
{
    ...
    if(isSequenceReached("*/")) {
        ...
        goForward(1);
        if(doesLineStartComment && (currentLine.find_first_not_of(" \t", charNum + 1) == string::npos))
            lineEndsInCommentOnly = true;
        if(peekNextChar() == '}' && previousCommandChar != ';' &&
           !isBracketType(bracketTypeStack->back(), ARRAY_TYPE) && !isInPreprocessor &&
           isOkToBreakBlock(bracketTypeStack->back())) {
            isInLineBreak = true;
            shouldBreakLineAtNextChar = true;
        }
        ...
        while(charNum + 1 < (int)currentLine.length() && !isLineReady && currentLine[charNum + 1] != '\t' &&
              currentLine.compare(charNum + 1, 2, "*/") != 0) {
            currentChar = currentLine[++charNum];
            appendCurrentChar();
        }
...
void ASFormatter::formatCommentOpener()
{
    ...
    if(spacePadNum != 0 && !isInLineBreak) adjustComments();
    formattedLineCommentNum = formattedLine.length();

    // must be done BEFORE appendSequence
    if(previousCommandChar == '{' && !isImmediatelyPostComment && !isImmediatelyPostLineComment) {
        if(bracketFormatMode == NONE_MODE) {
            // should a run-in statement be attached?
            if(currentLineBeginsWithBracket) formatRunIn();
            ...
            // should a run-in statement be attached?
            if(formattedLine.length() > 0 && formattedLine[0] == '{') formatRunIn();
        }
    ...
    if(shouldBreakBlocks) {
        ...
        if(doesLineStartComment && !isImmediatelyPostEmptyLine && !isImmediatelyPostCommentOnly &&
           previousCommandChar != '{') {
            checkForHeaderFollowingComment(currentLine.substr(charNum - 1));
        }
    }

    if(previousCommandChar == '}') currentHeader = NULL;
}
...
void ASFormatter::formatLineCommentOpener()
{
    ...
    if((int)currentLine.length() > charNum + 2 && currentLine[charNum + 2] == '\xf2') // check for windows line marker
        isAppendPostBlockEmptyLineRequested = false;
    ...
    if(lineCommentNoIndent == false && spacePadNum != 0 && !isInLineBreak) adjustComments();
    formattedLineCommentNum = formattedLine.length();
    ...
    if(previousCommandChar == '{' && !isImmediatelyPostComment && !isImmediatelyPostLineComment) {
        if(bracketFormatMode == NONE_MODE) {
            ...
            if(formattedLine.length() > 0 && formattedLine[0] == '{') isInLineBreak = true;
        } else {
            if(currentLineBeginsWithBracket) isInLineBreak = true;
        }
    }
    ...
    if(formattedLine.compare(0, 2, "//") == 0) lineIsLineCommentOnly = true;
    ...
    if(shouldBreakBlocks) {
        ...
        if(lineIsLineCommentOnly && previousCommandChar != '{' && !isImmediatelyPostEmptyLine &&
           !isImmediatelyPostCommentOnly) {
            checkForHeaderFollowingComment(currentLine.substr(charNum - 1));
        }
    }
    ...
    if(getIndentString() == "\t" && lineCommentNoIndent) {
        while(charNum + 1 < (int)currentLine.length() && currentLine[charNum + 1] == '\t') {
            currentChar = currentLine[++charNum];
            appendCurrentChar();
        }
    }
...
void ASFormatter::formatQuoteBody()
{
    ...
    if(isInQuote && currentChar != '\\') {
        while(charNum + 1 < (int)currentLine.length() && currentLine[charNum + 1] != quoteChar &&
              currentLine[charNum + 1] != '\\') {
            currentChar = currentLine[++charNum];
            appendCurrentChar();
        }
...
void ASFormatter::formatQuoteOpener()
{
    ...
    if(isSharpStyle() && previousChar == '@') isInVerbatimQuote = true;

    // a quote following a bracket is an array
    if(previousCommandChar == '{' && !isImmediatelyPostComment && !isImmediatelyPostLineComment &&
       isNonInStatementArray && !isBracketType(bracketTypeStack->back(), SINGLE_LINE_TYPE) &&
...
int ASFormatter::getNextLineCommentAdjustment()
{
    ...
    if(lastBracket != string::npos) return (lastBracket - charNum); // return a negative number
    return 0;
}

// for console build only
LineEndFormat ASFormatter::getLineEndFormat() const { return lineEnd; }
...
{
    assert(foundClosingHeader && previousNonWSChar == '}');
    if(charNum < 1) return 2;
    size_t lastBracket = currentLine.rfind('}', charNum - 1);
    if(lastBracket == string::npos) return 2;
    return 0;
}
...
{
    // get the last legal word (may be a number)
    if(currPos == 0) return string();

    size_t end = line.find_last_not_of(" \t", currPos - 1);
    if(end == string::npos || !isLegalNameChar(line[end])) return string();

    int start; // start of the previous word
    for(start = end; start > -1; start--) {
        if(!isLegalNameChar(line[start]) || line[start] == '.') break;
    }
    start++;
...
{
    assert(foundClosingHeader && previousNonWSChar == '}');
    if(bracketFormatMode == BREAK_MODE || bracketFormatMode == RUN_IN_MODE || shouldAttachClosingBracket) {
        isInLineBreak = true;
    } else if(bracketFormatMode == NONE_MODE) {
        if(shouldBreakClosingHeaderBrackets || getBracketIndent() || getBlockIndent()) {
            isInLineBreak = true;
        } else {
            appendSpacePad();
            // is closing bracket broken?
            size_t i = currentLine.find_first_not_of(" \t");
            if(i != string::npos && currentLine[i] == '}') isInLineBreak = false;

            if(shouldBreakBlocks) isAppendPostBlockEmptyLineRequested = false;
        }
    }
    // bracketFormatMode == ATTACH_MODE, LINUX_MODE, STROUSTRUP_MODE
        ...
        if(shouldBreakClosingHeaderBrackets || getBracketIndent() || getBlockIndent()) {
            ...
            if(!previousLineIsEmpty && previousLineIsOneLineBlock == 0) {
                isInLineBreak = false;
                appendSpacePad();
...
bool ASFormatter::addBracketsToStatement()
{
    ...
    if(currentHeader != &AS_IF && currentHeader != &AS_ELSE && currentHeader != &AS_FOR && currentHeader != &AS_WHILE &&
       currentHeader != &AS_DO && currentHeader != &AS_FOREACH)
        return false;
    ...
    if(isCharPotentialHeader(currentLine, charNum))
        if(findHeader(headers) != NULL) return false;

    // find the next semi-colon
    size_t nextSemiColon = charNum;
    if(currentChar != ';') nextSemiColon = findNextChar(currentLine, ';', charNum + 1);
    if(nextSemiColon == string::npos) return false;
    ...
    if(!shouldAddOneLineBrackets) {
        size_t lastText = formattedLine.find_last_not_of(" \t");
        if((formattedLine.length() - 1) - lastText > 1) formattedLine.erase(lastText + 1);
    }
    return true;
}
...
size_t ASFormatter::findNextChar(string& line, char searchChar, int searchStart /*0*/)
{
    ...
    for(i = searchStart; i < line.length(); i++) {
        if(line.compare(i, 2, "//") == 0) return string::npos;
        if(line.compare(i, 2, "/*") == 0) {
            size_t endComment = line.find("*/", i + 2);
            if(endComment == string::npos) return string::npos;
            i = endComment + 2;
            if(i >= line.length()) return string::npos;
        }
        if(line[i] == '\'' || line[i] == '\"') {
            char quote = line[i];
            while(i < line.length()) {
                size_t endQuote = line.find(quote, i + 1);
                if(endQuote == string::npos) return string::npos;
                i = endQuote;
...
bool ASFormatter::isStructAccessModified(string& firstLine, size_t index) const
{
    ...
    while(sourceIterator->hasMoreLines()) {
        ...
        for(size_t i = 0; i < nextLine_.length(); i++) {
            if(isWhiteSpace(nextLine_[i])) continue;
            if(nextLine_.compare(i, 2, "/*") == 0) isInComment_ = true;
            if(isInComment_) {
            ...
            if(isInQuote_) {
                if(nextLine_[i] == quoteChar_) isInQuote_ = false;
                continue;
            }
            ...
            if(isCharPotentialHeader(nextLine_, i)) {
                if(findKeyword(nextLine_, i, AS_PUBLIC) || findKeyword(nextLine_, i, AS_PRIVATE) ||
...
bool ASFormatter::isExecSQL(string& line, size_t index) const
{
    if(line[index] != 'e' && line[index] != 'E') // quick check to reject most
        return false;
    string word;
    if(isCharPotentialHeader(line, index)) word = getCurrentWord(line, index);
    for(size_t i = 0; i < word.length(); i++)
        word[i] = (char)toupper(word[i]);
    if(word != "EXEC") return false;
    size_t index2 = index + word.length();
    index2 = line.find_first_not_of(" \t", index2);
    if(index2 == string::npos) return false;
    word.erase();
    if(isCharPotentialHeader(line, index2)) word = getCurrentWord(line, index2);
    for(size_t i = 0; i < word.length(); i++)
        word[i] = (char)toupper(word[i]);
    if(word != "SQL") return false;
    return true;
}
...
void ASFormatter::trimContinuationLine()
{
    ...
    if(leadingSpaces > 0 && len > 0) {
        ...
        for(i = 0; (i < len) && (i + continuationIncrementIn < leadingSpaces); i++) {
            ...
            {
                if(i < continuationIncrementIn) leadingSpaces = i + tabIncrementIn;
        ...
        if((int)continuationIncrementIn == tabIncrementIn)
            ...
            string newLine;
            int leadingChars = 0;
            if((int)leadingSpaces > tabIncrementIn) leadingChars = leadingSpaces - tabIncrementIn;
...
{
    return (header == &AS_ELSE || header == &AS_CATCH || header == &AS_FINALLY);
}
...
void ASFormatter::checkIfTemplateOpener()
{
    ...
    for(size_t i = charNum; i < currentLine.length(); i++) {
        ...
        if(isWhiteSpace(currentChar_)) continue;
        ...
        if(currentChar_ == '<') {
            ...
            if(currentChar_ == '(')
                ...
                parenDepth_--;
            continue;
        } else if(currentLine.compare(i, 2, "&&") == 0 || currentLine.compare(i, 2, "||") == 0) {
...
void ASFormatter::updateFormattedLineSplitPoints(char appendedChar)
{
    ...
    if(!isOkToSplitFormattedLine()) return;

    char nextChar = peekNextChar();

    // don't split before or after a bracket
    if(appendedChar == '{' || appendedChar == '}' || previousNonWSChar == '{' || previousNonWSChar == '}' ||
    ...
    if(appendedChar == '[' || appendedChar == ']' || previousNonWSChar == '[' || nextChar == '[' || nextChar == ']')
        return;
    ...
    if(isWhiteSpace(appendedChar)) {
        if(nextChar != ')' // empty parens
           ...
           &&
           !(nextChar == '*' && !isCharPotentialOperator(previousNonWSChar) && pointerAlignment == PTR_ALIGN_TYPE) &&
           !(nextChar == '&' && !isCharPotentialOperator(previousNonWSChar) &&
             (referenceAlignment == REF_ALIGN_TYPE ||
              (referenceAlignment == REF_SAME_AS_PTR && pointerAlignment == PTR_ALIGN_TYPE))) &&
           !(nextChar == '(' && !isCharPotentialOperator(previousNonWSChar)) // not an operator followed by a paren
           &&
           !(currentChar == '(' &&
    ...
    // unpadded operators may split before the operator (counts as whitespace)
        ...
        if(charNum > 0 && (isLegalNameChar(currentLine[charNum - 1]) || currentLine[charNum - 1] == ')')) {
    ...
    // unpadded closing parens may split after the paren (counts as whitespace)
        ...
        char adjacentChar = ' ';
        if(charNum + 1 < (int)currentLine.length()) adjacentChar = currentLine[charNum + 1];
        if(previousNonWSChar != '(' // empty parens
           &&
           adjacentChar != ' ' && adjacentChar != ';' && adjacentChar != ',' && adjacentChar != '.') {
            if(maxWhiteSpace == 0 || formattedLine.length() < maxCodeLength)
                maxWhiteSpace = formattedLine.length();
            else
...
{
    return (appendedChar == '+' || appendedChar == '-' || appendedChar == '=' || appendedChar == ':' ||
            appendedChar == '?');
}
...
bool ASFormatter::isOkToSplitFormattedLine()
{
    ...
    if(shouldKeepLineUnbroken || isInLineComment || isInComment || isInQuote || isInBlParen || isInPreprocessor ||
       isInExecSQL || isInAsm || isInAsmOneLine || isInAsmBlock || isInTemplate)
        return false;

    if(!isOkToBreakBlock(bracketTypeStack->back()) || isBracketType(bracketTypeStack->back(), ARRAY_TYPE)) {
    ...
    if(formattedLine.length() > maxCodeLength && !isLineReady) {
        ...
        if(splitPoint > 0) {
            ...
            if(formattedLineCommentNum != string::npos) {
                ...
                if(formattedLineCommentNum == string::npos) formattedLineCommentNum = formattedLine.find("/*");
            }
        }
    }
}
...
size_t ASFormatter::findFormattedLineSplitPoint(int sequenceLength) const
{
    ...
    if(formattedLine.length() == maxCodeLength + 1 && (currentChar == ';' || currentChar == ' ')) return 0;
    ...
    if(splitPoint < minCodeLength) splitPoint = 0;
    // use maxParen instead of whitespace if it is long enough
    if(splitPoint == 0 && maxParen > minCodeLength &&
       (maxParen > maxWhiteSpace || maxParen > (maxCodeLength * .7) || maxWhiteSpace > maxCodeLength))
        splitPoint = maxParen;
    // use whitespace or paren if available
    if(splitPoint == 0) splitPoint = maxWhiteSpace;
    if(splitPoint == 0 && maxParen > 0) splitPoint = maxParen;
    // replace split point with first available break point
    if(splitPoint < minCodeLength) {
        splitPoint = string::npos;
        if(maxSemiPending > 0 && maxSemiPending < splitPoint) splitPoint = maxSemiPending;
        //		if (maxAndOrPending > 0 && maxAndOrPending < splitPoint)	// TODO: Fix This
        //			splitPoint = maxAndOrPending;
        if(maxCommaPending > 0 && maxCommaPending < splitPoint) splitPoint = maxCommaPending;
        if(maxParenPending > 0 && maxParenPending < splitPoint) splitPoint = maxParenPending;
        if(maxWhiteSpacePending > 0 && maxWhiteSpacePending < splitPoint) splitPoint = maxWhiteSpacePending;
        if(splitPoint == string::npos) splitPoint = 0;
    }

    // Don't split if near the end.
    int remainingCurrent = currentLine.length() - (charNum + sequenceLength);
    if(remainingCurrent == 0) {
        if(formattedLine.length() <= maxCodeLength || formattedLine.length() <= splitPoint ||
           (splitPoint >= maxCodeLength && formattedLine.length() <= maxCodeLength + 2))
            splitPoint = 0;
    }
...
bool ASFormatter::computeChecksumIn(const string& currentLine_)
{
    for(size_t i = 0; i < currentLine_.length(); i++)
        if(!isWhiteSpace(currentLine_[i])) checksumIn += currentLine_[i];
    return true;
}
 ...
 * @return   checksumIn.
 */
size_t ASFormatter::getChecksumIn() const { return checksumIn; }
...
bool ASFormatter::computeChecksumOut(const string& beautifiedLine)
{
    for(size_t i = 0; i < beautifiedLine.length(); i++)
        if(!isWhiteSpace(beautifiedLine[i])) checksumOut += beautifiedLine[i];
    return true;
}
 ...
 */
bool ASFormatter::getIsLineReady() const { return isLineReady; }
 ...
 * @return   checksumOut.
 */
size_t ASFormatter::getChecksumOut() const { return checksumOut; }

/**
 * Return the difference in checksums.
 * If zero all is okay.
 */
int ASFormatter::getChecksumDiff() const { return checksumOut - checksumIn; }

// for unit testing
int ASFormatter::getFormatterFileType() const { return formatterFileType; }
...
const string* ASFormatter::getFollowingOperator() const
{
    ...
    while(nextNum < currentLine.length()) {
        if(!isLegalNameChar(currentLine[nextNum]) && !isWhiteSpace(currentLine[nextNum])) break;
        nextNum++;
    }

    if(nextNum >= currentLine.length() || !isCharPotentialOperator(currentLine[nextNum]) ||
...
bool ASFormatter::isArrayOperator() const
{
    ...
    if(currentLine[nextNum] == ',' || currentLine[nextNum] == '}' || currentLine[nextNum] == ')' ||
       currentLine[nextNum] == '(')
        return true;
    return false;
}
