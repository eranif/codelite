...
public:
    enum { DbConnTypeMySQL, DbConnTypePgSQL };
...
public:
    ...
    void SetServer(const wxString& server) { this->m_server = server; }
    void SetPort(const int port) { this->m_port = port; }
    void SetUsername(const wxString& username) { this->m_username = username; }
    const wxString& GetConnectionName() const { return m_connectionName; }
    ...
    void SetDefaultDatabase(const wxString& defaultDatabase) { this->m_defaultDatabase = defaultDatabase; }
    const wxString& GetDefaultDatabase() const { return m_defaultDatabase; }
    void SetConnectionType(int connectionType) { this->m_connectionType = connectionType; }
    int GetConnectionType() const { return m_connectionType; }

    bool IsValid() const
    {
        return !m_connectionName.IsEmpty() && !m_server.IsEmpty() && !m_username.IsEmpty() && !m_password.IsEmpty();
    }
};
