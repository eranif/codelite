...
int DumpClass::DumpTable(wxTextFile* pFile, Table* pTab)
{
    ...
    if ((pFile->IsOpened()) && (pTab != NULL)) {
        ...
        if (pDbLayer) {
            // DatabaseResultSet* pResult = pDbLayer->RunQueryWithResults(wxString::Format(wxT("SELECT %s FROM %s.%s"),
            // cols.c_str(),pTab->GetParentName().c_str(), pTab->GetName().c_str()));
            DatabaseResultSet* pResult = pDbLayer->RunQueryWithResults(
                m_pDbAdapter->GetDefaultSelect(cols, pTab->GetParentName(), pTab->GetName()));

            while (pResult->Next()) {
                if (n == 0)
                    pFile->AddLine(startLine);
                rowCount++;
                ...
                for (auto node = pTab->GetFirstChildNode(); node; node = node->GetNext()) {
                    ...
                    if (pCol) {
                        ...
                        if (pCol->GetType()->GetUniversalType() == IDbType::dbtTYPE_TEXT) {
                            ...
                            dataLine +=
                                wxString::Format(wxT("'%s'"), pResult->GetResultDate(colIndex).FormatDate().c_str());
                            ...
                            if (pResult->GetResultBool(colIndex))
                                dataLine += wxT("true");
                            else
                                dataLine += wxT("false");
