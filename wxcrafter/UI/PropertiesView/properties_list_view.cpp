...
void PropertiesListView::ConstructProjectSettings()
{
    ...
    AddBoolProp(
        _("Generate Window ID"),
        wxcProjectMetadata::Get().IsUseEnum(),
    ...
    AddIntegerProp(_("First Window ID"),
                   wxcProjectMetadata::Get().GetFirstWindowId(),
    ...
    AddBoolProp(_("Generate Translatable Strings"),
                wxcProjectMetadata::Get().IsUseUnderscoreMacro(),
    ...
    AddBoolProp(_("Add wxWidgets Handlers if missing"),
                wxcProjectMetadata::Get().IsAddHandlers(),
        ...
        if (property->GetType() == PT_CATEGORY) {
            ...
            if (intProp) {
                if (intProp->IsUninitialized()) {
                    ...
                    pgProp = AddIntegerProp(property->GetLabelForUI(),
                                            wxCrafter::ToNumber(property->GetValue(), -1),
                                            property->GetTooltip());
                }
            }

        } else if (property->GetType() == PT_COMBOX) {
            pgProp = AddComboxProp(
                property->GetLabelForUI(), property->GetOptions(), property->GetValue(), property->GetTooltip());
            ...
            pgProp = AddFlags(property->GetLabelForUI(),
                              property->GetOptions(),
                              property->GetOptionsValues(),
                              property->GetValueLong(),
                              property->GetTooltip());
        }
...
wxPGProperty*
PropertiesListView::AddChoiceProp(const wxString& label, const wxArrayString& value, int sel, const wxString& tip)
{
...
wxPGProperty* PropertiesListView::AddComboxProp(const wxString& label,
        ...
        if (pb) {
            ...
            if (pb->GetLabelForUI() == _("Name")) {
                if (m_wxcWidget->GetTopLevel() && !m_wxcWidget->GetTopLevel()->IsNameUnique(pgp->GetValueAsString())) {
                    ...
                    ::wxMessageBox(
                        _("A control with this name already exists!"), "wxCrafter", wxICON_WARNING | wxOK | wxCENTER);
                    return;
                }
            }
...
wxPGProperty* PropertiesListView::AddFlags(
