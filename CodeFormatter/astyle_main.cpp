 ...
 */
template <typename T> string ASStreamIterator<T>::nextLine(bool emptyLineWasDeleted)
{
...
template <typename T> string ASStreamIterator<T>::peekNextLine()
{
    ...
    if(peekStart == 0) peekStart = inStream->tellg();
...
template <typename T> bool ASStreamIterator<T>::getLineEndChange(int lineEndFormat) const
{
    assert(lineEndFormat == LINEEND_DEFAULT || lineEndFormat == LINEEND_WINDOWS || lineEndFormat == LINEEND_LINUX ||
...
void ASConsole::correctMixedLineEnds(ostringstream& out)
{
    ...
    if(strcmp(outputEOL, "\r\n") == 0) lineEndFormat = LINEEND_WINDOWS;
    if(strcmp(outputEOL, "\n") == 0) lineEndFormat = LINEEND_LINUX;
    if(strcmp(outputEOL, "\r") == 0) lineEndFormat = LINEEND_MACOLD;
...
void ASConsole::formatFile(const string& fileName_)
{
    ...
    while(formatter.hasMoreLines()) {
        ...
        if(filesAreIdentical) {
            if(streamIterator.checkForEmptyLine) {
                if(nextLine.find_first_not_of(" \t") != string::npos) filesAreIdentical = false;
    ...
    if(!filesAreIdentical || streamIterator.getLineEndChange(lineEndFormat)) {
        ...
        if(!isFormattedOnly) printMsg(_("Unchanged  %s\n"), displayName);
        filesUnchanged++;
    }
...
// for unit testing
vector<bool> ASConsole::getExcludeHitsVector() { return excludeHitsVector; }

// for unit testing
vector<string> ASConsole::getExcludeVector() { return excludeVector; }

// for unit testing
vector<string> ASConsole::getFileName() { return fileName; }

// for unit testing
vector<string> ASConsole::getFileNameVector() { return fileNameVector; }

// for unit testing
vector<string> ASConsole::getFileOptionsVector() { return fileOptionsVector; }

int ASConsole::getFilesUnchanged() { return filesUnchanged; }

int ASConsole::getFilesFormatted() { return filesFormatted; }

bool ASConsole::getIgnoreExcludeErrors() { return ignoreExcludeErrors; }

bool ASConsole::getIgnoreExcludeErrorsDisplay() { return ignoreExcludeErrorsDisplay; }

bool ASConsole::getIsFormattedOnly() { return isFormattedOnly; }

string ASConsole::getLanguageID() const { return localizer.getLanguageID(); }

bool ASConsole::getIsQuiet() { return isQuiet; }

bool ASConsole::getIsRecursive() { return isRecursive; }

bool ASConsole::getIsVerbose() { return isVerbose; }

bool ASConsole::getLineEndsMixed() { return lineEndsMixed; }

bool ASConsole::getNoBackup() { return noBackup; }

string ASConsole::getOptionsFileName() { return optionsFileName; }

bool ASConsole::getOptionsFileRequired() { return optionsFileRequired; }

// for unit testing
vector<string> ASConsole::getOptionsVector() { return optionsVector; }

string ASConsole::getOrigSuffix() { return origSuffix; }

string ASConsole::getParam(const string& arg, const char* op) { return arg.substr(strlen(op)); }

bool ASConsole::getPreserveDate() { return preserveDate; }
...
FileEncoding ASConsole::readFile(const string& fileName_, stringstream& in) const
{
    ...
    if(!fin) error("Cannot open input file", fileName_.c_str());
    char* data = new (nothrow) char[blockSize];
    if(!data) error("Cannot allocate memory for input file", fileName_.c_str());
    fin.read(data, sizeof(data));
    if(fin.bad()) error("Cannot read input file", fileName_.c_str());
    size_t dataSize = static_cast<size_t>(fin.gcount());
    FileEncoding encoding = detectEncoding(data, dataSize);
    if(encoding == UTF_32BE || encoding == UTF_32LE) error(_("Cannot process UTF-32 encoding"), fileName_.c_str());
    ...
    while(dataSize) {
        if(encoding == UTF_16LE || encoding == UTF_16BE) {
            ...
            if(!utf8Out) error("Cannot allocate memory for utf-8 conversion", fileName_.c_str());
...
void ASConsole::setOptionsFileName(string name) { optionsFileName = name; }

void ASConsole::setOptionsFileRequired(bool state) { optionsFileRequired = state; }

void ASConsole::setOrigSuffix(string suffix) { origSuffix = suffix; }
...
void ASConsole::setOutputEOL(LineEndFormat lineEndFormat, const char* currentEOL)
{
    if(lineEndFormat == LINEEND_DEFAULT) {
        ...
        if(strlen(prevEOL) == 0) strcpy(prevEOL, outputEOL);
        ...
        if(strcmp(prevEOL, outputEOL) != 0) filesAreIdentical = false;
    }
}
...
void ASConsole::getFileNames(const string& directory, const string& wildcard)
{
    ...
    do {
        ...
        if(findFileData.cFileName[0] == '.' || (findFileData.dwFileAttributes & FILE_ATTRIBUTE_HIDDEN) ||
    ...
    if(dwError != ERROR_NO_MORE_FILES) error("Error processing directory", directory.c_str());
...
string ASConsole::getNumberFormat(int num, size_t lcid) const
{
    ...
    if(decBuf == NULL) return number;
    ::GetLocaleInfo(lcid, LOCALE_SDECIMAL, decBuf, decSize);
    size_t i = formattedNum.rfind(decBuf);
    delete[] decBuf;
    if(i != string::npos) formattedNum.erase(i);
    if(!formattedNum.length()) formattedNum = "0";
    return formattedNum;
}
...
string ASConsole::getCurrentDirectory(const string& fileName_) const
{
    ...
    if(currdir == NULL) error("Cannot find file", fileName_.c_str());
    return string(currdir);
}
...
void ASConsole::getFileNames(const string& directory, const string& wildcard)
{
    ...
    while((entry = readdir(dp)) != NULL) {
        ...
        // skip hidden or read only
        if(entry->d_name[0] == '.' || !(statbuf.st_mode & S_IWUSR)) continue;
        // if a sub directory and recursive, save sub directory
    ...
    if(firstEntry < fileName.size()) sort(&fileName[firstEntry], &fileName[fileName.size()]);

    // recurse into sub directories
    // if not doing recursive, subDirectory is empty
    if(subDirectory.size() > 1) sort(subDirectory.begin(), subDirectory.end());
    for(unsigned i = 0; i < subDirectory.size(); i++) {
        getFileNames(subDirectory[i], wildcard);
    }
...
string ASConsole::getNumberFormat(int num, const char* groupingArg, const char* separator) const
{
    ...
    if(grouping == 0) grouping = number.length();
    while(i > 0) {
        ...
        i -= grouping;
        if(i < 0) i = 0;
        if(i > 0) formattedNum.insert(0, separator);
        number.erase(i);
        // update grouping
        if(groupingArg[ig] != '\0' && groupingArg[ig + 1] != '\0') grouping = groupingArg[++ig];
    }
    return formattedNum;
}
...
void ASConsole::getFilePaths(string& filePath)
{
    ...
    hasWildcard = false;
    if(targetFilename.find_first_of("*?") != string::npos) hasWildcard = true;
    ...
    if(hasWildcard || isRecursive)
        ...
        struct stat statbuf;
        if(stat(entryFilepath.c_str(), &statbuf) == 0 && (statbuf.st_mode & S_IFREG)) fileName.push_back(entryFilepath);
    }
    ...
    for(size_t ix = 0; ix < excludeHitsVector.size(); ix++) {
        if(excludeHitsVector[ix] == false) {
            ...
            if(!ignoreExcludeErrorsDisplay) {
                ...
                if(!ignoreExcludeErrors) fprintf(stderr, _("Exclude (unmatched)  %s\n"), excludeVector[ix].c_str());
            }
        }
    }

    if(excludeErr && !ignoreExcludeErrors) {
        if(hasWildcard && !isRecursive) fprintf(stderr, "%s\n", _("Did you intend to use --recursive"));
        error();
    }
    ...
    if(hasWildcard) printSeparatingLine();
}

bool ASConsole::fileNameVectorIsEmpty() { return fileNameVector.empty(); }

bool ASConsole::isOption(const string& arg, const char* op) { return arg.compare(op) == 0; }
...
bool ASConsole::isParamOption(const string& arg, const char* option)
{
    ...
    if(retVal && strlen(option) == 1 && arg.length() > 1)
        if(!isdigit((unsigned char)arg[1])) retVal = false;
    return retVal;
}
...
bool ASConsole::isPathExclued(const string& subPath)
{
    ...
    for(size_t i = 0; i < excludeVector.size(); i++) {
        ...
        if(subPath.length() < exclude.length()) continue;
        ...
        if(compareStart > 0) {
            ...
            if(lastPathChar != g_fileSeparator) continue;
        }
...
void ASConsole::processFiles()
{
    ...
    if(isVerbose) printVerboseStats(startTime);
}
...
void ASConsole::processOptions(vector<string>& argvOptions)
{
    ...
    for(size_t i = 0; i < argvOptions.size(); i++) {
        ...
        if(isOption(arg, "-I") || isOption(arg, "--ascii")) {
            ...
            if(optionsFileName.compare("") == 0) setOptionsFileName(" ");
        } else if(isOption(arg, "-h") || isOption(arg, "--help") || isOption(arg, "-?")) {
            printHelp();
            exit(EXIT_SUCCESS);
        } else if(isOption(arg, "-V") || isOption(arg, "--version")) {
    ...
    if(shouldParseOptionsFile) {
        if(optionsFileName.compare("") == 0) {
            ...
            if(env != NULL) setOptionsFileName(env);
        }
        if(optionsFileName.compare("") == 0) {
            char* env = getenv("HOME");
            if(env != NULL) setOptionsFileName(string(env) + "/.astylerc");
        }
        if(optionsFileName.compare("") == 0) {
            char* env = getenv("USERPROFILE");
            if(env != NULL) setOptionsFileName(string(env) + "/astylerc");
        }
        if(optionsFileName.compare("") != 0) standardizePath(optionsFileName);
    }
    ...
    if(optionsFileName.compare("") != 0) {
        ...
        if(optionsIn) {
            ...
            ok = options.parseOptions(fileOptionsVector, string(_("Invalid option file options:")));
        } else {
            if(optionsFileRequired) error(_("Cannot open options file"), optionsFileName.c_str());
    ...
    ok = options.parseOptions(optionsVector, string(_("Invalid command line options:")));
    if(!ok) {
        (*_err) << options.getOptionErrors() << endl;
        (*_err) << _("For help on options type 'astyle -h'") << endl;
        error();
...
void ASConsole::standardizePath(string& path, bool removeBeginningSeparator /*false*/) const
{
    ...
    if(removeBeginningSeparator && (path[0] == g_fileSeparator)) path.erase(0, 1);
}
...
void ASConsole::printVerboseHeader() const
{
    ...
    // print options file
    if(!optionsFileName.empty()) printf(_("Using default options file %s\n"), optionsFileName.c_str());
}
...
bool ASConsole::stringEndsWith(const string& str, const string& suffix) const
{
    ...
    while(strIndex >= 0 && suffixIndex >= 0) {
        if(tolower(str[strIndex]) != tolower(suffix[suffixIndex])) return false;

        --strIndex;
        --suffixIndex;
    }
    // suffix longer than string
    if(strIndex < 0 && suffixIndex >= 0) return false;
    return true;
}

// Swap the two low order bytes of an integer value
// and convert 8 bit encoding to 16 bit.
int ASConsole::swap8to16bit(int value) const { return (((value & 0xFF) << 8) + (value >> 8)); }

// Swap the two low order bytes of a 16 bit integer value.
int ASConsole::swap16bit(int value) const { return (((value & 0xff) << 8) | ((value & 0xff00) >> 8)); }
    ...
    while(pRead < pEnd) {
        ...
        if(eState == eStart) {
            ...
            if(codePoint >= SURROGATE_FIRST_VALUE) {
                ...
                *pCur++ = static_cast<utf16>((encoding == UTF_16BE) ? swap8to16bit(lead) : lead);
                int trail = (codePoint & 0x3ff) + SURROGATE_TRAIL_FIRST;
                *pCur++ = static_cast<utf16>((encoding == UTF_16BE) ? swap8to16bit(trail) : trail);
            } else {
                *pCur++ = static_cast<utf16>((encoding == UTF_16BE) ? swap8to16bit(codePoint) : codePoint);
            }
        }
    }
...
// Copyright (C) 2002 Scott Kirkwood.
// Modified for Artistic Style by Jim Pattee.
size_t ASConsole::Utf16ToUtf8(char* utf16In, size_t inLen, FileEncoding encoding, bool firstBlock, char* utf8Out) const
{
    typedef unsigned short utf16; // 16 bits
    typedef unsigned char ubyte;  // 8 bits
    enum { SURROGATE_LEAD_FIRST = 0xD800 };
...
int ASConsole::waitForRemove(const char* newFileName) const
{
    ...
    for(seconds = 1; seconds <= 20; seconds++) {
        sleep(1);
        if(stat(newFileName, &stBuf) != 0) break;
    }
    errno = 0;
    return seconds;
}
...
void ASConsole::writeFile(const string& fileName_, FileEncoding encoding, ostringstream& out) const
{
    ...
    struct stat stBuf;
    bool statErr = false;
    if(stat(fileName_.c_str(), &stBuf) == -1) statErr = true;
    ...
    if(!fout) error("Cannot open output file", fileName_.c_str());
    if(encoding == UTF_16LE || encoding == UTF_16BE) {
    ...
    if(preserveDate) {
        if(!statErr) {
            ...
            if(utime(fileName_.c_str(), &outBuf) == -1) statErr = true;
        }
        if(statErr) {
            perror("errno message");
...
bool ASOptions::parseOptions(vector<string>& optionsVector, const string& errorInfo)
{
    ...
    if(optionErrors.str().length() > 0) return false;
    return true;
}
...
void ASOptions::parseOption(const string& arg, const string& errorInfo)
{
    if(isOption(arg, "style=allman") || isOption(arg, "style=ansi") || isOption(arg, "style=bsd") ||
       isOption(arg, "style=break")) {
        ...
        if(styleParam.length() > 0) style = atoi(styleParam.c_str());
        if(style == 1)
            formatter.setFormattingStyle(STYLE_ALLMAN);
        else if(style == 2)
            formatter.setFormattingStyle(STYLE_JAVA);
    ...
    // must check for mode=cs before mode=c !!!
        ...
        if(spaceNumParam.length() > 0) spaceNum = atoi(spaceNumParam.c_str());
        ...
        if(tabNumParam.length() > 0) tabNum = atoi(tabNumParam.c_str());
        ...
        if(minIndentParam.length() > 0) minIndent = atoi(minIndentParam.c_str());
        ...
        if(maxIndentParam.length() > 0) maxIndent = atoi(maxIndentParam.c_str());
        if(maxIndent < 40)
            isOptionError(arg, errorInfo);
        ...
        if(styleParam.length() > 0) align = atoi(styleParam.c_str());
        if(align < 1 || align > 3)
            isOptionError(arg, errorInfo);
        else if(align == 1)
            formatter.setPointerAlignment(PTR_ALIGN_TYPE);
        ...
        if(maxLengthParam.length() > 0) maxLength = atoi(maxLengthParam.c_str());
        if(maxLength < 50)
            isOptionError(arg, errorInfo);
...
// end depreciated options ////////////////////////////////////////////////////////////////////////////////////////
    ...
    // Options used by only console ///////////////////////////////////////////////////////////////////////////////////
        ...
        if(suffixParam.length() > 0) g_console->updateExcludeVector(suffixParam);
        ...
        if(lineendParam.length() > 0) lineendType = atoi(lineendParam.c_str());
...
void ASOptions::importOptions(istream& in, vector<string>& optionsVector)
{
    ...
    while(in) {
        ...
        if(currentToken.length() != 0) optionsVector.push_back(currentToken);
    }
}

string ASOptions::getOptionErrors() { return optionErrors.str(); }

string ASOptions::getParam(const string& arg, const char* op) { return arg.substr(strlen(op)); }
...
bool ASOptions::isOption(const string arg, const char* op) { return arg.compare(op) == 0; }
...
{
    if(optionErrors.str().length() == 0) optionErrors << errorInfo << endl; // need main error message
...
Java_AStyleInterface_AStyleMain(JNIEnv* env, jobject obj, jstring textInJava, jstring optionsJava)
{
    ...
    if(textOut == NULL) return textErr;
                                           ...
                                           fpAlloc fpMemoryAlloc)  // pointer to memory allocation function
{
    ...
    if(!ok) fpErrorHandler(210, options.getOptionErrors().c_str());
...
extern "C" EXPORT const char* STDCALL AStyleGetVersion(void) { return g_version; }
