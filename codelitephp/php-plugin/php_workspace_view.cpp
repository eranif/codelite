...
#define CHECK_ID_FOLDER( id ) if ( !id->IsFolder() ) return
#define CHECK_ID_PROJECT( id ) if ( !id->IsProject() ) return
#define CHECK_ID_FILE( id ) if ( !id->IsFile() ) return
    ...
    , m_mgr(mgr)
{
    ...
    EventNotifier::Get()->Connect(wxEVT_CMD_EXECUTE_ACTIVE_PROJECT, wxCommandEventHandler(PHPWorkspaceView::OnRunActiveProject), NULL, this);
    EventNotifier::Get()->Connect(wxEVT_CMD_STOP_EXECUTED_PROGRAM, wxCommandEventHandler(PHPWorkspaceView::OnStopExecutedProgram), NULL, this);
    EventNotifier::Get()->Connect(wxEVT_CMD_IS_PROGRAM_RUNNING, wxCommandEventHandler(PHPWorkspaceView::OnIsProgramRunning), NULL, this);
    EventNotifier::Get()->Connect(wxEVT_ACTIVE_EDITOR_CHANGED, wxCommandEventHandler(PHPWorkspaceView::OnEditorChanged), NULL, this);
    EventNotifier::Get()->Connect(wxEVT_PHP_FILE_RENAMED, PHPEventHandler(PHPWorkspaceView::OnFileRenamed), NULL, this);
    EventNotifier::Get()->Connect(wxEVT_PHP_WORKSPACE_RENAMED, PHPEventHandler(PHPWorkspaceView::OnWorkspaceRenamed), NULL, this);
...
{
    EventNotifier::Get()->Disconnect(wxEVT_CMD_EXECUTE_ACTIVE_PROJECT, wxCommandEventHandler(PHPWorkspaceView::OnRunActiveProject), NULL, this);
    EventNotifier::Get()->Disconnect(wxEVT_CMD_STOP_EXECUTED_PROGRAM, wxCommandEventHandler(PHPWorkspaceView::OnStopExecutedProgram), NULL, this);
    EventNotifier::Get()->Disconnect(wxEVT_CMD_IS_PROGRAM_RUNNING, wxCommandEventHandler(PHPWorkspaceView::OnIsProgramRunning), NULL, this);
    EventNotifier::Get()->Disconnect(wxEVT_ACTIVE_EDITOR_CHANGED, wxCommandEventHandler(PHPWorkspaceView::OnEditorChanged), NULL, this);
    EventNotifier::Get()->Disconnect(wxEVT_PHP_FILE_RENAMED, PHPEventHandler(PHPWorkspaceView::OnFileRenamed), NULL, this);
    EventNotifier::Get()->Disconnect(wxEVT_PHP_WORKSPACE_RENAMED, PHPEventHandler(PHPWorkspaceView::OnWorkspaceRenamed), NULL, this);
}
    ...
    if(item.IsOk()) {
        ...
        if(data) {
            ...
            case ItemData::Kind_Workspace: {
                ...
                menu.AppendSeparator();
                menu.Append(XRCID("new_project"),       _("Add a new project...")); // This will trigger codelite's "New Project Wizard"
                menu.AppendSeparator();
...
void PHPWorkspaceView::UnLoadWorkspace()
{
    m_treeCtrlView->DeleteAllItems();
}
...
void PHPWorkspaceView::DoAddFolderFiles(const wxString& project, const wxString& folder, const wxTreeItemId& folderTreeItemId, wxStringSet_t* filesAdded)
{
...
void PHPWorkspaceView::OnAddFile(wxCommandEvent& e)
{
    ...
    if(dlg.ShowModal() == wxID_OK) {
        ...
        if ( !paths.Item(0).StartsWith(pProject->GetFilename().GetPath()) ) {
            ::wxMessageBox(_("Can not add file to project. File must be under the project directory tree"), "CodeLite", wxOK|wxCENTER|wxICON_WARNING);
            return;
        }
...
void PHPWorkspaceView::DoAddFilesToFolder(const wxArrayString& paths, PHPProject::Ptr_t pProject, bool notify)
{
    ...
    wxTreeItemId folderItem = EnsureFolderExists(DoGetProjectItem(pProject->GetName()), folder_path, pProject->GetName());
...
void PHPWorkspaceView::OnRemoveFile(wxCommandEvent& e)
{
...
void PHPWorkspaceView::DoDeleteSelectedFileItem()
{
    ...
    msg << _("Removing file '") << wxFileName(itemData->GetFile()).GetFullName() << "' from project '" << itemData->GetProjectName() << "'\n"
...
void PHPWorkspaceView::OnRunActiveProject(wxCommandEvent& e)
{
...
void PHPWorkspaceView::OnNewClass(wxCommandEvent& e)
{
    ...
    if ( dlg.ShowModal() == wxID_OK ) {
        ...
        pFolder = pProject->AddFolder( PHPFolder::GetFolderPathFromFileFullPath(pcd.GetFilepath().GetFullPath(), pProject->GetFilename().GetPath()) );
        CHECK_PTR_RET(pFolder);
        
        folderId = EnsureFolderExists(DoGetProjectItem(pProject->GetName()), pFolder->GetPathRelativeToProject(), pProject->GetName());
        DoAddFileWithContent(folderId, pcd.GetFilepath(), pcd.ToString(EditorConfigST::Get()->GetOptions()->GetEOLAsString(), "    "));
    }
}

void PHPWorkspaceView::DoAddFileWithContent(const wxTreeItemId& folderId, const wxFileName& filename, const wxString& content)
{
...
void PHPWorkspaceView::OnWorkspaceOpenUI(wxUpdateUIEvent& event)
{
    event.Enable( PHPWorkspace::Get()->IsOpen() );
}
...
wxTreeItemId PHPWorkspaceView::DoGetSingleSelection()
{
    return m_treeCtrlView->GetFocusedItem();
}
...
wxTreeItemId PHPWorkspaceView::EnsureFolderExists(const wxTreeItemId& projectItem, const wxString& path, const wxString& project)
{
    PHPProject::Ptr_t pProject = PHPWorkspace::Get()->GetProject(project);
    ...
    for(size_t i=0; i<parts.GetCount(); ++i) {
        ...
        if ( !HasFolderWithName(parent, parts.Item(i), parent ) ) {
            ...
            parent = m_treeCtrlView->AppendItem(parent, 
