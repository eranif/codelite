...
void VimCommand::completing_command(wxChar ch)
{
    ...
    case COMMAND_PART::MOD_NUM:
        ...
        if(ch < '9' && ch > '0' && m_baseCommand != 'r' && m_baseCommand != 'f' && m_baseCommand != 'F' &&
            ...
            (m_externalCommand == 'f' || m_externalCommand == 'F' || m_externalCommand == 't' || m_externalCommand == 'T'))) {
...
bool VimCommand::command_call_visual_line_mode()
{
    ...
    case COMMANDVI::y: {
        ...
        if (beginLine > endLine) { std::swap(beginLine, endLine); }
        
        m_ctrl->SetSelection(m_ctrl->PositionFromLine(beginLine),
                                    m_ctrl->GetLineEndPosition(endLine));
