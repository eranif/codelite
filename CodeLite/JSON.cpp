...
{
    auto json = nlohmann::ordered_json::parse(text.mb_str(wxConvUTF8).data(), nullptr, false);
...
JSON::JSON(const wxFileName& filename)
{
    ...
    auto json = nlohmann::ordered_json::parse(content.ToStdString(wxConvUTF8), nullptr, false);
    if (json.is_discarded()) {
        return;
    }
    m_json = std::make_shared<nlohmann::ordered_json>(std::move(json));
...
{
    if (!m_json || !isObject() || !element.m_json) {
        return *this;
    }
