 ...
 */
template <typename T> int ASStreamIterator<T>::getStreamLength() const { return static_cast<int>(streamLength); }
...
template <typename T> streamoff ASStreamIterator<T>::tellg() { return inStream->tellg(); }
...
ASConsole::ASConsole(ASFormatter& formatterArg)
...
// for unit testing
ostream* ASConsole::getErrorStream() const { return errorStream; }

void ASConsole::setErrorStream(ostream* errStreamPtr) { errorStream = errStreamPtr; }
...
void ASConsole::displayLastError()
{
    ...
    FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, nullptr, lastError,
...
void ASConsole::getFileNames(const string& directory, const string& wildcard)
{
    ...
    HANDLE hFind = FindFirstFileA(firstFile.c_str(), &findFileData);
...
string ASConsole::getNumberFormat(int num, size_t lcid) const
{
    ...
    int outSize = ::GetNumberFormatA(lcid, 0, number.c_str(), nullptr, nullptr, 0);
    char* outBuf = new(nothrow) char[outSize];
    if(outBuf == nullptr)
        return number;
    ::GetNumberFormatA(lcid, 0, number.c_str(), nullptr, outBuf, outSize);
    string formattedNum(outBuf);
    delete[] outBuf;
    // remove the decimal
    int decSize = ::GetLocaleInfoA(lcid, LOCALE_SDECIMAL, nullptr, 0);
...
void ASConsole::launchDefaultBrowser(const char* filePathIn /*nullptr*/) const
{
    ...
    for(size_t i = 0; i < pathsLen; i++) {
        ...
        if(htmlDefaultPath.length() > 0 && htmlDefaultPath[htmlDefaultPath.length() - 1] == g_fileSeparator)
...
string ASConsole::getNumberFormat(int num, const char* groupingArg, const char* separator) const
{
    ...
    while(i > 0) {
        ...
        number.erase(i);
        // update grouping
        if(groupingArg[ig] != '\0' && groupingArg[ig + 1] != '\0')
...
void ASConsole::launchDefaultBrowser(const char* filePathIn /*nullptr*/) const
{
    ...
    while(searchDir != nullptr) {
        ...
        if(searchPath.length() > 0 && searchPath[searchPath.length() - 1] != g_fileSeparator)
...
bool ASConsole::fileNameVectorIsEmpty() const { return fileNameVector.empty(); }
...
void ASConsole::writeFile(const string& fileName_, FileEncoding encoding, ostringstream& out) const
{
    ...
    if(encoding == UTF_16LE || encoding == UTF_16BE) {
        ...
        size_t utf16Len =
            utf8_16.utf8ToUtf16(const_cast<char*>(out.str().c_str()), out.str().length(), isBigEndian, utf16Out);
                                ...
                                fpAlloc fpMemoryAlloc) const // memory allocation function)
{
    ...
    char* utf8Out = AStyleMain(utf8In, utf8Options);
    // finished with these
    delete[] utf8In;
    delete[] utf8Options;
    utf8In = nullptr;
...
string ASOptions::getOptionErrors() const { return optionErrors.str(); }
...
int ASEncoding::swap16bit(int value) const { return (((value & 0xff) << 8) | ((value & 0xff00) >> 8)); }
...
size_t ASEncoding::utf16ToUtf8(char* utf16In, size_t inLen, bool isBigEndian, bool firstBlock, char* utf8Out) const
{
    int nCur16 = 0;
    int nCur = 0;
...
// the function name is constructed from method names in the calling java program
extern "C" EXPORT jstring STDCALL Java_AStyleInterface_AStyleMain(JNIEnv* env, jobject obj, jstring textInJava,
                                                                  jstring optionsJava)
{
 ...
 * No /EXPORT is required for x64
 */
const char* AStyleGetVersion(void) { return g_version; }
char* AStyleMain(const char* pSourceIn, // the source to be formatted
                  const char* pOptions)  // memory allocation function
{
    if(pSourceIn == nullptr) {
        return nullptr;
    }
    ...
    char* pTextOut = (char*)malloc((long)textSizeOut + 1); // call memory allocation function
    if(pTextOut == nullptr) {
        return nullptr;
    }
