...
extern WXDLLIMPEXP_CL void get_variables(const std::string& in,
                                         VariableList& li,
...
extern WXDLLIMPEXP_CL void
get_functions(const std::string& in, FunctionList& li, const std::map<std::string, std::string>& ignoreTokens);
...
public:
    ...
    ~CppLexer() { ::cl_scope_lex_clean(); }
    int lex() const { return cl_scope_lex(); }
    int line_number() const { return cl_scope_lineno; }
    std::string text() const { return cl_scope_text; }
};
