...
#ifndef __LLDBLocalsModel_GUARD__H__
#define __LLDBLocalsModel_GUARD__H__
 ...
 * To convert between a wxDataViewItem to a LLDBLocalsModel_Item class, simply use:
 * @code
 * LLDBLocalsModel_Item* itemData = reinterpret_cast<LLDBLocalsModel_Item*>( item.GetID() );
 * if ( itemData ) {
 ...
 * @endcode
 */
class LLDBLocalsModel_Item
{
protected:
    wxVector<wxVariant>        m_data;
    LLDBLocalsModel_Item*           m_parent;
    wxVector<LLDBLocalsModel_Item*> m_children;
    bool                       m_isContainer;
    wxClientData*              m_clientData;

public:
    LLDBLocalsModel_Item()
        : m_parent(NULL)
        , m_isContainer(false)
        , m_clientData(NULL)
    {}
    virtual ~LLDBLocalsModel_Item() {
        if ( m_clientData ) {
            delete m_clientData;
            m_clientData = NULL;
        }
        ...
        wxVector<LLDBLocalsModel_Item*> tmpChildren = m_children;
     ...
     * @param child
     */
    void DeleteChild(LLDBLocalsModel_Item* child) {
        wxVector<LLDBLocalsModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
     ...
     * @param child
     */
    void RemoveChild(LLDBLocalsModel_Item* child) {
        wxVector<LLDBLocalsModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
        if ( iter != m_children.end() ) {
            m_children.erase(iter);
        }
     ...
     */
    void AddChild(LLDBLocalsModel_Item* child) {
    ...
    // Setters / Getters
    void SetChildren(const wxVector<LLDBLocalsModel_Item*>& children) {
        this->m_children = children;
    }
    void SetData(const wxVector<wxVariant>& data) {
        this->m_data = data;
    }
    void SetParent(LLDBLocalsModel_Item* parent) {
        this->m_parent = parent;
    }
    const wxVector<LLDBLocalsModel_Item*>& GetChildren() const {
        return m_children;
    }
    wxVector<LLDBLocalsModel_Item*>& GetChildren() {
        return m_children;
    }
    ...
    LLDBLocalsModel_Item* GetParent() {
        return m_parent;
    }
...
class LLDBLocalsModel : public wxDataViewModel
{
protected:
    wxVector<LLDBLocalsModel_Item*> m_data;
    unsigned int m_colCount;

public:
    LLDBLocalsModel();
    virtual ~LLDBLocalsModel();
    ...
    void SetData(const wxVector<LLDBLocalsModel_Item*> data) {
        this->m_data = data;
    }
    unsigned int GetColCount() const {
        return m_colCount;
    }
    const wxVector<LLDBLocalsModel_Item*>& GetData() const {
        return m_data;
    }
    wxVector<LLDBLocalsModel_Item*>& GetData() {
        return m_data;
    }
...
#endif // __LLDBLocalsModel_GUARD__H__
