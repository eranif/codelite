    ...
    , m_isDefault(false)
{
    ...
    if (node) {
        ...
        if (regexType == kRegexGNU) {
            ...
            AddPattern(
                kSevError, "^windres: ([a-zA-Z:]{0,2}[ a-zA-Z\\\\.0-9_/\\+\\-]+) *:([0-9]+): syntax error", 1, 2);
...
wxXmlNode* Compiler::ToXml() const
{
    ...
    for (const auto& pattern : m_errorPatterns) {
        ...
        error->AddAttribute(wxT("ColumnIndex"), pattern.columnIndex);
        XmlUtils::SetNodeContent(error, pattern.pattern);
        node->AddChild(error);
    }
    ...
    for (const auto& pattern : m_warningPatterns) {
        ...
        warning->AddAttribute(wxT("ColumnIndex"), pattern.columnIndex);
        XmlUtils::SetNodeContent(warning, pattern.pattern);
...
bool Compiler::IsMatchesPattern(CmpInfoPattern& pattern,
                                eSeverity severity,
                                const wxString& line,
                                PatternMatch* match_result) const
{
    ...
    if (!pattern.re->IsValid()) {
        clWARNING() << "Regex pattern:" << pattern.pattern << "is not valid!" << endl;
        return false;
    }
    ...
    if (!pattern.columnIndex.ToCLong(&colIndex))
        return false;

    if (!pattern.lineNumberIndex.ToCLong(&lineIndex))
        return false;

    if (!pattern.fileNameIndex.ToCLong(&fileIndex))
        return false;
