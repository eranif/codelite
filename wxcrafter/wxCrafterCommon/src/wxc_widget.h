...
public:
    ...
    /**
     * @brief if the controls requires additional files, generate their content/names
    ...
    void ChildrenXRC(wxString& text, XRC_TYPE type) const;
    ...
    bool CanMoveRight() const { return (GetAdjacentSiblingSizer() != nullptr); }
    ...
    void InsertBefore(wxcWidget* item, const wxcWidget* insertBefore);
    void InsertAfter(wxcWidget* item, const wxcWidget* insertAfter);
    ...
    // If non-nullptr, direction will hold whether above or below
    wxcWidget* GetAdjacentSiblingSizer(bool* isAbove = nullptr) const;
     ...
     * of 'p'
     */
    bool IsDirectOrIndirectChildOf(const wxcWidget* p) const;
     ...
     */
    bool IsNameUnique(const wxString& name) const;
    ...
    static wxcWidget* Create(int id);
    static wxcWidget* CreateFromJSON(const JSONItem& json);
    ...
    static EventsDatabase& GetCommonEvents();
...
protected:
     ...
     * #endif
     */
    void SetIfBlockCond(const wxString& preprocessorCondition);
    /**
     * @brief wrap 'code' with the proper #if / #endif block
     * if needed. The code will get wrapped if !GetPreprocessorCondition().IsEmpty()
     * @param code
     */
    void WrapInIfBlockIfNeeded(wxString& code) const;

    const wxString& GetPreprocessorCondition() const { return m_preprocessorCondition; }
    ...
    wxString XRCBitmap(const wxString& label = "bitmap") const;
    ...
    static void DoClearFlags(MapStyles_t& mp);

    static void DoEnableStyle(wxcWidget::MapStyles_t& mp, const wxString& style, bool enable);
