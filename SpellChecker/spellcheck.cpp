...
{
    m_topWin->Disconnect( IDM_SETTINGS, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( SpellCheck::OnSettings ), NULL, this );
    m_topWin->Disconnect( XRCID( s_doCheckID ), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( SpellCheck::OnCheck ), NULL, this );
    m_topWin->Disconnect( XRCID( s_contCheckID ), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( SpellCheck::OnContinousCheck ), NULL, this );
...
clToolBar* SpellCheck::CreateToolBar( wxWindow* parent )
{
    if( m_mgr->AllowToolbar() ) {
        ...
        if( size == 24 ) {
            ...
            m_pToolbar->AddTool( XRCID( s_contCheckID ), _( "Check continuous" ), bitmap2, _( "Run continuous check" ), wxITEM_CHECK );
        } else {
            ...
            wxBitmap bitmap2( wxImage( png_stream2, wxBITMAP_TYPE_PNG ) );
            m_pToolbar->AddTool( XRCID( s_contCheckID ), _( "Check continuous" ), bitmap2, _( "Start continuous check" ), wxITEM_CHECK );
        }
    ...
    parent->Connect( XRCID( s_doCheckID ), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( SpellCheck::OnCheck ), NULL, this );
    parent->Connect( XRCID( s_contCheckID ), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( SpellCheck::OnContinousCheck ), NULL, this );
    return m_pToolbar;
}
// ------------------------------------------------------------
void SpellCheck::CreatePluginMenu( wxMenu* pluginsMenu )
{
    ...
    m_topWin->Connect( IDM_SETTINGS, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( SpellCheck::OnSettings ), NULL, this );
}
// ------------------------------------------------------------
wxMenu* SpellCheck::CreateSubMenu()
{
    m_pSubMenu = new wxMenu();

    wxMenuItem* pItem( NULL );
    pItem = new wxMenuItem( m_pSubMenu, XRCID( s_doCheckID ), _( "Check..." ), _( "Check..." ), wxITEM_NORMAL );
    m_pSubMenu->Append( pItem );
    pItem = new wxMenuItem( m_pSubMenu, XRCID( s_contCheckID ), _( "Check continuous" ), _( "Start continuous check" ), wxITEM_CHECK );
    m_pSubMenu->Append( pItem );
    return m_pSubMenu;
}
// ------------------------------------------------------------
...
void SpellCheck::SetCheckContinuous( bool value )
{
    ...
    if( value ) {
        ...
        if( m_pSubMenu )
            m_pSubMenu->Check( XRCID( s_contCheckID ), true );
    } else {
        if( m_timer.IsRunning() )
            m_timer.Stop();
        ...
        if( m_pSubMenu )
            m_pSubMenu->Check( XRCID( s_contCheckID ), false );
    }
}
// ------------------------------------------------------------
