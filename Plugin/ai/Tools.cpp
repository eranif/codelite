...
namespace
{
ollama::FunctionResult Ok(std::optional<wxString> text)
{
    ollama::FunctionResult result{.isError = false};
    if (text.has_value()) {
...
ollama::FunctionResult Err(std::optional<wxString> text)
{
    ollama::FunctionResult result{.isError = true};
    if (text.has_value()) {
        result.text = text.value().ToStdString(wxConvUTF8);
    }
    return result;
