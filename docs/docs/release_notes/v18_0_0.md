...
- Debug Adapter Protocol Client (DAP): fixed bug where init param `cwd` was set to empty value when launching the server
...
- Zoom Navigator: the marker is now centered.
...
- Source code formatter: apply the environment variables before executing the formatting tool
...
- Truncate code completion tooltip text to a manageable size (1K)
...
- Fixed: editor flickers after formatter executions
...
- SpellChecker: use `UTF8Length()` method to calculate real length of tokens, this should fix problems when using non ASCII chars in the document
...
- Fixed: there is a noticeable lag when typing while the function argument tooltip is shown
...
- Fixed: when LSP tries to place many markers ("diagnostics") on the editor, it causes an excessive reads from the disk which can cause a noticeable lag
