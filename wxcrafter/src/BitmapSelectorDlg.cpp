...
{
    wxArrayString arr = {wxT("wxART_ERROR"),
                         wxT("wxART_QUESTION"),
                         wxT("wxART_WARNING"),
    ...
    if (wxCrafter::IsArtProviderBitmap(selection, artID, clientID, sizeHint)) {
        ...
        if (where != wxNOT_FOUND) {
            m_choiceArtClientID->SetSelection(where);
        }

        where = m_choiceArtID->FindString(artID);
        if (where != wxNOT_FOUND) {
            m_choiceArtID->SetSelection(where);
        }

        where = m_choiceSize->FindString(sizeHint);
        if (where != wxNOT_FOUND) {
            m_choiceSize->SetSelection(where);
        }
...
BitmapSelectorDlg::~BitmapSelectorDlg()
{
    wxcSettings::Get().EnableFlag(
        wxcSettings::DONT_USE_RELATIVE_BITMAPS, !m_checkBoxConvertToRelativePath->IsChecked());
}
...
void BitmapSelectorDlg::OnBrowseFile(wxCommandEvent& event)
{
    ...
    if (fnBitmap.IsRelative()) {
        fnBitmap.MakeAbsolute(wxcProjectMetadata::Get().GetProjectPath());
    }
    ...
    wxString newPath = ::wxFileSelector(
        _("Select bitmap"), fnBitmap.GetPath(), fnBitmap.GetFullName(), wxEmptyString, wildcard, wxFD_OPEN);
