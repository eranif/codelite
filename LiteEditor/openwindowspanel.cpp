    ...
    , m_workspaceClosing(false)
{
    ...
    m_toolbar->AddTool(wxID_SORT_ASCENDING, _("Sort"), images->LoadBitmap("sort"), _("Sort Items"), wxITEM_CHECK);
    m_toolbar->Realize();
    ...
    EventNotifier::Get()->Connect(wxEVT_INIT_DONE, wxCommandEventHandler(OpenWindowsPanel::OnInitDone), nullptr, this);
...
OpenWindowsPanel::~OpenWindowsPanel()
{
    ...
    Clear();
    EventNotifier::Get()->Disconnect(wxEVT_INIT_DONE, wxCommandEventHandler(OpenWindowsPanel::OnInitDone), nullptr, this);
    EventNotifier::Get()->Disconnect(
        wxEVT_ACTIVE_EDITOR_CHANGED, wxCommandEventHandler(OpenWindowsPanel::OnActiveEditorChanged), nullptr, this);
    EventNotifier::Get()->Disconnect(
        wxEVT_CMD_PAGE_CHANGED, wxCommandEventHandler(OpenWindowsPanel::OnActivePageChanged), nullptr, this);
    EventNotifier::Get()->Disconnect(
        wxEVT_ALL_EDITORS_CLOSED, wxCommandEventHandler(OpenWindowsPanel::OnAllEditorsClosed), nullptr, this);
...
void OpenWindowsPanel::OnInitDone(wxCommandEvent& event)
{
    ...
    EventNotifier::Get()->Bind(wxEVT_ACTIVE_EDITOR_CHANGED, &OpenWindowsPanel::OnActiveEditorChanged, this);
    EventNotifier::Get()->Bind(wxEVT_CMD_PAGE_CHANGED, &OpenWindowsPanel::OnActiveEditorChanged, this);
    EventNotifier::Get()->Bind(wxEVT_ALL_EDITORS_CLOSED, &OpenWindowsPanel::OnAllEditorsClosed, this);
...
wxVariant OpenWindowsPanel::PrepareValue(const clTab& tab, bool* isModified)
{
    ...
    *isModified = false;
    ...
    auto image = clGetManager()->GetStdIcons()->GetBitmapForFile(title, false);
    if (!image.IsOk()) {
        image = clGetManager()->GetStdIcons()->GetBitmapForFile("file.txt", false);
    }
    ...
    wxDataViewIconText iconText(title, image);
    wxVariant value;
    value << iconText;
    return value;
}
