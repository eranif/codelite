...
void ObjectPreview::FromJSON(const JSONElement& json)
{
    ...
    if(json.hasNamedObject("properties")) {
        JSONElement props = json.namedObject("properties");
        int size = props.arraySize();
        for(int i = 0; i < size; ++i) {
            JSONElement prop = props.arrayItem(i);
...
JSONElement ObjectPreview::ToJSON(const wxString& name) const
{
    ...
    if(!m_properties.empty()) {
        JSONElement arr = JSONElement::createArray("properties");
