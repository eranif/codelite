...
public:
    ...
    const char* getPipePath() const { return _pipePath.c_str(); }

    ZNP_ERROR getLastError() { return _lastError; }

    void setLastError(ZNP_ERROR code) { _lastError = code; }

protected:
    void setPipePath(const char* path) { _pipePath = path; }

    void setPipeHandle(PIPE_HANDLE handle) { _pipeHandle = handle; }
