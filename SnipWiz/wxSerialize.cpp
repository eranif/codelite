...
int wxSerialize::LoadChunkHeader(int expheader)
{
    ...
    if (CanLoad()) {
        ...
        if (hdr != expheader) {
            LogError(wxSERIALIZE_ERR_ILL,
                     wxSERIALIZE_ERR_STR_WRONGCHUNK_s1_s2,
                     GetHeaderName(expheader),
                     GetHeaderName(hdr));
            return -1;
        }
    }
...
wxUint8 wxSerialize::LoadUint8() { return LoadChar(); }
...
void wxSerialize::SaveUint8(wxUint8 value) { SaveChar(value); }
...
int wxSerialize::LogError(int err, int msgcode, const wxString& s1, const wxString& s2)
{
    ...
    if (m_errorCode == wxSERIALIZE_ERR_OK && err != wxSERIALIZE_ERR_OK) {
        ...
        case wxSERIALIZE_ERR_STR_HEADER_s1_s2:
            error << wxT("Wrong header in start of stream, expected header '") << s1 << wxT(" and got '") << s2
                  << wxT("'");
            break;

        case wxSERIALIZE_ERR_STR_WRONGVERSION_s1_s2:
            error << wxT("Invalid version in stream, got v") << s1 << wxT(" but expected v") << s2 << wxT(" or higher");
            break;
        ...
        case wxSERIALIZE_ERR_STR_NOHEADER_s1:
            error << wxT("No valid header found in stream but expected header '") << s1 << wxT("'");
            break;
        ...
        case wxSERIALIZE_ERR_STR_WRONGCHUNK_s1_s2:
            error << wxT("Expected chunk item of type '") << s1 << wxT("' but got type '") << s2 << wxT("'");
            break;

        case wxSERIALIZE_ERR_STR_MEMORY_s1:
            error << wxT("Memory allocation error. Cannot allocate ") << s1 << wxT(" bytes");
            break;
...
/*static*/ long wxSerialize::GetLibVersionMajor() { return WXSERIALIZE_MAJOR_VERSION; }

/*static*/ long wxSerialize::GetLibVersionMinor() { return WXSERIALIZE_MINOR_VERSION; }
