...
bool PHPWorkspace::IsOpen() const
{
    return m_workspaceFile.IsOk() && m_workspaceFile.FileExists();
}
...
void PHPWorkspace::AddFiles(const wxString& project, const wxString& folder, const wxArrayString& files, wxArrayString &filesAdded)
{
...
bool PHPWorkspace::IsProjectExists(const wxString& project)
{
    return HasProject(project);
}

const PHPProject::Map_t& PHPWorkspace::GetProjects() const
{
    return m_projects;
}
...
bool PHPWorkspace::RunProject(bool debugging, const wxString& projectName, const wxString& xdebugSessionName)
{
    ...
    if(!m_executor.Exec(projectToRun, xdebugSessionName, debugging)) {
        ::wxMessageBox(_("Failed to execute project"), wxT("CodeLite"), wxOK|wxICON_ERROR|wxCENTER, wxTheApp->GetTopWindow());
        return false;
    }
    return true;
}
...
PHPProject::Ptr_t PHPWorkspace::GetActiveProject() const
{
...
{
    wxMessageDialog dlg(FRAME, _("Workspace file modified externally. Would you like to reload the workspace?"), "CodeLite", wxYES_NO|wxCENTER);
