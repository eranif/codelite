...
struct WXDLLIMPEXP_SDK LineAndPattern {
    int line_number{0};
    wxString pattern;
};

struct WXDLLIMPEXP_SDK GrepResult {
    size_t count{0};
    std::map<wxString, std::vector<LineAndPattern>> matches;
};

/**
 * Parses grep output and groups matches by file.
 */
WXDLLIMPEXP_SDK GrepResult ParseGrepOutput(const wxArrayString& output_arr);
