...
unsigned int CppCheck::checkFile(const std::string& filename, const std::string& cfgname, std::istream& fileStream)
{
    ...
    try {
        ...
        for(simplecpp::OutputList::const_iterator it = outputList.begin(); it != outputList.end(); ++it) {
            ...
            if(err) {
                ...
                ErrorLogger::ErrorMessage errmsg(callstack, "", Severity::error, it->msg, "syntaxError", false);
                reportErr(errmsg);
                return mExitCode;
            }
        }
        ...
        if(mSettings.dump) {
            ...
            if(fdump.is_open()) {
                ...
                fdump << "  <platform"
                      << " name=\"" << mSettings.platformString() << '\"' << " char_bit=\"" << mSettings.char_bit
                      << '\"' << " short_bit=\"" << mSettings.short_bit << '\"' << " int_bit=\"" << mSettings.int_bit
                      << '\"' << " long_bit=\"" << mSettings.long_bit << '\"' << " long_long_bit=\""
                      << mSettings.long_long_bit << '\"' << " pointer_bit=\""
                      << (mSettings.sizeof_pointer * mSettings.char_bit) << '\"' << "/>\n";
                fdump << "  <rawtokens>" << std::endl;
                for(unsigned int i = 0; i < files.size(); ++i)
                    fdump << "    <file index=\"" << i << "\" name=\"" << ErrorLogger::toxml(files[i]) << "\"/>"
                          << std::endl;
        ...
        for(std::set<std::string>::const_iterator it = configurations.begin(); it != configurations.end(); ++it) {
            ...
            try {
                ...
                ErrorLogger::ErrorMessage errmsg(locationList, mTokenizer.list.getSourceFilePath(), Severity::error,
        ...
        if(!hasValidConfig && configurations.size() > 1 && mSettings.isEnabled(Settings::INFORMATION)) {
            ...
            msg += "\nThis file is not analyzed. Cppcheck failed to extract a valid configuration. The tested "
            ...
            locationList.push_back(loc);
            ErrorLogger::ErrorMessage errmsg(locationList, loc.getfile(), Severity::information, msg,
    ...
    if(!mSettings.jointSuppressionReport &&
       (mSettings.isEnabled(Settings::INFORMATION) || mSettings.checkConfiguration)) {
...
void CppCheck::internalError(const std::string& filename, const std::string& msg)
{
    ...
    if(mSettings.isEnabled(Settings::INFORMATION)) {
        ...
        ErrorLogger::ErrorMessage errmsg(callstack, emptyString, Severity::information, fullmsg, "internalError",
                                         false);
...
static const char* pcreErrorCodeToString(const int pcreExecRet)
{
    ...
    case PCRE_ERROR_BADLENGTH:
        return "This error is given if pcre_exec() is called with a negative value for the length argument "
    ...
    case PCRE_ERROR_JIT_BADOPTION:
        return "This error is returned when a pattern that was successfully studied using a JIT compile "
               ...
               "also given for invalid options. See the pcrejit documentation for more details "
...
void CppCheck::executeRules(const std::string& tokenlist, const Tokenizer& tokenizer)
{
    ...
    for(std::list<Settings::Rule>::const_iterator it = mSettings.rules.begin(); it != mSettings.rules.end(); ++it) {
        ...
        if(!re) {
            if(pcreCompileErrorStr) {
                ...
                const ErrorLogger::ErrorMessage errmsg(std::list<ErrorLogger::ErrorMessage::FileLocation>(),
                                                       emptyString, Severity::error, msg, "pcre_compile", false);
        ...
        if(pcreStudyErrorStr) {
            ...
            const ErrorLogger::ErrorMessage errmsg(std::list<ErrorLogger::ErrorMessage::FileLocation>(), emptyString,
                                                   Severity::error, msg, "pcre_study", false);
        ...
        while(pos < (int)str.size()) {
            ...
            if(pcreExecRet < 0) {
                ...
                if(!errorMessage.empty()) {
                    const ErrorLogger::ErrorMessage errmsg(
                        std::list<ErrorLogger::ErrorMessage::FileLocation>(), emptyString, Severity::error,
                        std::string("pcre_exec failed: ") + errorMessage, "pcre_exec", false);
            ...
            const ErrorLogger::ErrorMessage errmsg(callStack, tokenizer.list.getSourceFilePath(), rule.severity,
                                                   summary, rule.id, false);
...
void CppCheck::tooManyConfigsError(const std::string& file, const std::size_t numberOfConfigurations)
{
    ...
    if(file.empty())
        msg << " configurations. Use --force to check all configurations. For more details, use "
               "--enable=information.\n";
    ...
    ErrorLogger::ErrorMessage errmsg(loclist, emptyString, Severity::information, msg.str(), "toomanyconfigs", CWE398,
                                     false);
...
void CppCheck::purgedConfigurationMessage(const std::string& file, const std::string& configuration)
{
    ...
    ErrorLogger::ErrorMessage errmsg(loclist, emptyString, Severity::information,
                                     "The configuration '" + configuration +
                                         "' was not checked because its code equals another one.",
                                     "purgedConfiguration", false);
...
void CppCheck::reportErr(const ErrorLogger::ErrorMessage& msg)
{
    ...
    if(!mSettings.nofail.isSuppressed(errorMessage) &&
       (mUseGlobalSuppressions || !mSettings.nomsg.isSuppressed(errorMessage)))
        mExitCode = 1;
...
void CppCheck::reportOut(const std::string& outmsg) { mErrorLogger.reportOut(outmsg); }
...
void CppCheck::reportStatus(unsigned int /*fileindex*/, unsigned int /*filecount*/, std::size_t /*sizedone*/,
...
void CppCheck::analyseWholeProgram(const std::string& buildDir, const std::map<std::string, std::size_t>& files)
{
    ...
    while(std::getline(fin, filesTxtLine)) {
        ...
        for(const tinyxml2::XMLElement* e = rootNode->FirstChildElement(); e; e = e->NextSiblingElement()) {
            ...
            if(!checkClassAttr)
                continue;
            for(std::list<Check*>::const_iterator it = Check::instances().begin(); it != Check::instances().end();
                ++it) {
