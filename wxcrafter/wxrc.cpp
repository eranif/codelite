...
static bool NodeContainsFilename(wxXmlNode* node)
{
    ...
    if(name == wxT("bitmap") || name == wxT("bitmap2")) return true;

    if(name == wxT("icon")) return true;
    ...
    if(name == wxT("object")) {
        ...
        if(klass == wxT("wxBitmap") || klass == wxT("wxIcon") || klass == wxT("data")) return true;
    }
...
void wxcXmlResourceCmp::OutputGettext()
{
    ...
    if(!m_outputCppFile) fout.Detach();
}
