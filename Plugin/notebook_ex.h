...
public:
     ...
     * \param notify set this to true if you wish to receive wxEVT_COMMAND_BOOK_PAGE_CLOSING &
     * wxEVT_COMMAND_BOOK_PAGE_CLOSED
     */
...
public:
     ...
     */
    NotebookEvent(wxEventType commandType = wxEVT_NULL,
                  int winid = 0,
                  size_t nSel = (size_t)-1,
                  size_t nOldSel = (size_t)-1)
     ...
     */
    void SetSelection(size_t s) { sel = s; }

    /**
     * \param s - index of previously selected page
     */
    void SetOldSelection(size_t s) { oldsel = s; }

    /// Returns the index of currently selected page
    size_t GetSelection() { return sel; }

    /// Returns the index of previously selected page
    size_t GetOldSelection() { return oldsel; }
...
#define EVT_BOOK_SWAP_PAGES(winid, fn) wx__DECLARE_EVT1(wxEVT_COMMAND_BOOK_SWAP_PAGES, winid, NotebookEventHandler(fn))

#define EVT_BOOK_BG_DCLICK(winid, fn) wx__DECLARE_EVT1(wxEVT_COMMAND_BOOK_BG_DCLICK, winid, NotebookEventHandler(fn))
