...
#ifndef __LocalsViewModel_GUARD__H__
#define __LocalsViewModel_GUARD__H__
 ...
 * To convert between a wxDataViewItem to a LocalsViewModel_Item class, simply use:
 * @code
 * LocalsViewModel_Item* itemData = reinterpret_cast<LocalsViewModel_Item*>( item.GetID() );
 * if ( itemData ) {
 ...
 * @endcode
 */
class LocalsViewModel_Item
{
protected:
    wxVector<wxVariant>        m_data;
    LocalsViewModel_Item*           m_parent;
    wxVector<LocalsViewModel_Item*> m_children;
    bool                       m_isContainer;
    wxClientData*              m_clientData;

public:
    LocalsViewModel_Item()
        : m_parent(NULL)
        , m_isContainer(false)
        , m_clientData(NULL)
    {}
    virtual ~LocalsViewModel_Item() {
        if ( m_clientData ) {
            delete m_clientData;
            m_clientData = NULL;
        }
        ...
        wxVector<LocalsViewModel_Item*> tmpChildren = m_children;
     ...
     * @param child
     */
    void DeleteChild(LocalsViewModel_Item* child) {
        wxVector<LocalsViewModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
     ...
     * @param child
     */
    void RemoveChild(LocalsViewModel_Item* child) {
        wxVector<LocalsViewModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
        if ( iter != m_children.end() ) {
            m_children.erase(iter);
        }
     ...
     */
    void AddChild(LocalsViewModel_Item* child) {
    ...
    // Setters / Getters
    void SetChildren(const wxVector<LocalsViewModel_Item*>& children) {
        this->m_children = children;
    }
    void SetData(const wxVector<wxVariant>& data) {
        this->m_data = data;
    }
    void SetParent(LocalsViewModel_Item* parent) {
        this->m_parent = parent;
    }
    const wxVector<LocalsViewModel_Item*>& GetChildren() const {
        return m_children;
    }
    wxVector<LocalsViewModel_Item*>& GetChildren() {
        return m_children;
    }
    ...
    LocalsViewModel_Item* GetParent() {
        return m_parent;
    }
...
class LocalsViewModel : public wxDataViewModel
{
protected:
    wxVector<LocalsViewModel_Item*> m_data;
    unsigned int m_colCount;

public:
    LocalsViewModel();
    virtual ~LocalsViewModel();
    ...
    void SetData(const wxVector<LocalsViewModel_Item*> data) {
        this->m_data = data;
    }
    unsigned int GetColCount() const {
        return m_colCount;
    }
    const wxVector<LocalsViewModel_Item*>& GetData() const {
        return m_data;
    }
    wxVector<LocalsViewModel_Item*>& GetData() {
        return m_data;
    }
...
#endif // __LocalsViewModel_GUARD__H__
