                                           ...
                                           const int& cursorPosition, const int& selStart, const int& selEnd) const
{
    ...
    GenerateClangFormatFile(fileName);
    command << " -style=file";
    command << " " << filePath;
    return command;
}
...
void FormatOptions::GenerateClangFormatFile(const wxFileName& fileName) const
{
    // If we have a .clang-format file, use it
    wxFileName clang_format_file;
    wxString oldContent;
    if(HasConfigForFile(fileName, ".clang-format", clang_format_file)) {
        // check if this file was generated by CodeLite
        if(FileUtils::ReadFileContent(clang_format_file, oldContent) &&
           !oldContent.Contains(CLANG_FORMAT_CONF_FILE_HEADER)) {
            // this was file was not generated by CodeLite
            return;
        }
    }
    ...
    fileContent << CLANG_FORMAT_CONF_FILE_HEADER << "\n";
    fileContent << "# DO NOT EDIT THIS FILE\n";
    fileContent << "BasedOnStyle: ";

    if(m_clangFormatOptions & kClangFormatChromium) {
        fileContent << "Chromium";
    ...
    fileContent << ClangGlobalSettings();

    if(FileExtManager::IsJavascriptFile(fileName)) {
        fileContent << "Language : JavaScript"
                    << "\n";

    } else if(FileExtManager::IsCxxFile(fileName)) {
        fileContent << "Language : Cpp"
                    << "\n";
    ...
    fileContent << "AlignEscapedNewlinesLeft: " << ClangFlagToBool(kAlignEscapedNewlinesLeft) << "\n";
    fileContent << "AlignTrailingComments : " << ClangFlagToBool(kAlignTrailingComments) << "\n";
    fileContent << "AllowAllParametersOfDeclarationOnNextLine : "
                << ClangFlagToBool(kAllowAllParametersOfDeclarationOnNextLine) << "\n";
    fileContent << "AllowShortFunctionsOnASingleLine : " << ClangFlagToBool(kAllowShortFunctionsOnASingleLine) << "\n";
    fileContent << "AllowShortBlocksOnASingleLine : " << ClangFlagToBool(kAllowShortBlocksOnASingleLine) << "\n";
    fileContent << "AllowShortLoopsOnASingleLine : " << ClangFlagToBool(kAllowShortLoopsOnASingleLine) << "\n";
    fileContent << "AllowShortIfStatementsOnASingleLine : " << ClangFlagToBool(kAllowShortIfStatementsOnASingleLine)
                << "\n";
    fileContent << "AlwaysBreakBeforeMultilineStrings : " << ClangFlagToBool(kAlwaysBreakBeforeMultilineStrings)
                << "\n";
    fileContent << "AlwaysBreakTemplateDeclarations : " << ClangFlagToBool(kAlwaysBreakTemplateDeclarations) << "\n";
    fileContent << "BinPackParameters : " << ClangFlagToBool(kBinPackParameters) << "\n";
    fileContent << "BreakBeforeBinaryOperators : " << ClangFlagToBool(kBreakBeforeBinaryOperators) << "\n";
    fileContent << "BreakBeforeTernaryOperators : " << ClangFlagToBool(kBreakBeforeTernaryOperators) << "\n";
    fileContent << "BreakConstructorInitializersBeforeComma : "
                << ClangFlagToBool(kBreakConstructorInitializersBeforeComma) << "\n";
    fileContent << "IndentCaseLabels : " << ClangFlagToBool(kIndentCaseLabels) << "\n";
    fileContent << "IndentFunctionDeclarationAfterType : " << ClangFlagToBool(kIndentFunctionDeclarationAfterType)
                << "\n";
    fileContent << "SpaceBeforeAssignmentOperators : " << ClangFlagToBool(kSpaceBeforeAssignmentOperators) << "\n";
    fileContent << "SpaceBeforeParens : " << (m_clangFormatOptions & kSpaceBeforeParens ? "Always" : "Never") << "\n";
    fileContent << "SpacesInParentheses : " << ClangFlagToBool(kSpacesInParentheses) << "\n";
    fileContent << "BreakBeforeBraces : " << ClangBreakBeforeBrace() << "\n";
    fileContent << "ColumnLimit : " << m_clangColumnLimit << "\n";
    fileContent << "PointerAlignment : " << (m_clangFormatOptions & kPointerAlignmentRight ? "Right" : "Left") << "\n";
    if(fileContent != oldContent) {
        if(clGetManager()->IsWorkspaceOpen()) {
            clang_format_file = clGetManager()->GetWorkspace()->GetFileName();
        } else {
            clang_format_file = fileName;
        }
        clang_format_file.SetFullName(".clang-format");
        clDEBUG() << "Generating .clang-format file..." << clang_format_file << endl;
        FileUtils::WriteFileContent(clang_format_file, fileContent);
    }
}
