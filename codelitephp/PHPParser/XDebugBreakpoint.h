...
public:
    ...
    public:
        Equal(const wxString& filename, int line)
            : m_fileName(filename)
            , m_line(line)
        {
        }
...
public:
    ...
    bool IsNull() const { return m_line == wxNOT_FOUND; }

    void SetBreakpointId(int breakpointId) { this->m_breakpointId = breakpointId; }
    int GetBreakpointId() const { return m_breakpointId; }
