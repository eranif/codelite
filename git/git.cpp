    ...
    , m_commandProcessor(NULL)
{
    ...
    llm::Manager::GetInstance().GetFunctionTable().Add(
...
bool GitPlugin::GenerateCommitMessage(const wxString& prompt, const wxString& model_name)
{
    ...
    auto collector = std::make_unique<llm::ResponseCollector>(this);
    ...
    collector->SetFinaliseCallback([this](llm::ResponseCollector* collector) {
        m_commitDialog->AppendCommitMessage(wxString::FromUTF8(collector->GetResponse()));
        m_commitDialog->SetCommitMessageGenerationCompleted();
    });

    auto cancel_token = std::make_shared<llm::CancellationToken>(10000);
    llm::Manager::GetInstance().Chat(this, prompt, cancel_token, llm::ChatOptions::kNoTools, model_name);
    return true;
}
