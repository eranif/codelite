...
void clMainFrame::CreateToolbars24()
{
    ...
    tb->AddSeparator();
    tb->AddTool(XRCID("dbg_enable_reverse_debug"), _("Reverse Debugging"), bmpLoader.LoadBitmap("rewind", 24),
        _("Enable Reverse Debugging"), wxITEM_CHECK);
    tb->Realize();
...
void clMainFrame::CreateNativeToolbar16()
{
    ...
    tb->AddSeparator();
    tb->AddTool(XRCID("dbg_enable_reverse_debug"), _("Reverse Debugging"), bmpLoader.LoadBitmap("rewind"),
        _("Enable Reverse Debugging"), wxITEM_CHECK);
    SetToolBar(tb);
    tb->Realize();
}
...
void clMainFrame::OnToggleReverseDebuggingUI(wxUpdateUIEvent& e)
{
    ...
    if(enable) {
        e.Enable(true);
        e.Check(DebuggerMgr::Get().GetActiveDebugger()->IsReverseDebuggingEnabled());
    } else {
        e.Enable(false);
    }
}
