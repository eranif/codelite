...
- Added a new **Claude Code** plugin that hosts Claude Code in its own tab with a toolbar, keyboard shortcut, and menu entry.
  ...
  - Tab title and theme changes now follow the Claude Code session correctly.
- Added a persisted **system prompt** for Chat AI: edit it any time from `Chat AI` → `Edit System Prompt...`. It's applied to every new agent session, and edits now take effect on the running session immediately, without a restart.
- Chat AI now automatically loads a workspace's `AGENTS.md` (or `CLAUDE.md` if there's no `AGENTS.md`) into its system context as soon as the workspace opens, giving the model project-specific instructions without manually running `/context`.
...
- The tab switcher (`Ctrl-Tab`) now shows each entry's icon in its own column, with a modified-file indicator prepended to the file name instead of overlaid on the icon.
...
- Fixed the Git Commit dialog not loading the file list, diff, and commit message for the first commit until it was manually re-selected.
- Fixed the Claude Code plugin not being enabled automatically for existing users after upgrading.
- Fixed mouse wheel scrolling not working reliably on the right pane of the side-by-side diff view.
- Fixed Chat AI forcing the view back to the bottom while streaming new output, even after the user had scrolled up to read earlier messages; the toolbar toggle is now a "Jump to latest" button, enabled only while scrolled away from the bottom.
- Fixed the Claude Code plugin not using the resolved SSH account when starting a session against a remote workspace.
- Fixed terminal keyboard input handling on Linux (GTK) and keyboard layout translation on Windows, and corrected terminal working-directory initialization on Windows.
