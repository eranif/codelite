...
// Load debuggers (*must* be after the plugins)
    ...
    wxArrayString hiddenItems = clConfig::Get().Read("ToolBarHiddenItems", wxArrayString{"No Entries"});
...
{
    m_mgr.SetFlags(wxAUI_MGR_TRANSPARENT_HINT | wxAUI_MGR_HINT_FADE | wxAUI_MGR_LIVE_RESIZE);
}
...
{
    wxUnusedVar(e);
    NewSseMCPDlg dlg{this};
    if (dlg.ShowModal() != wxID_OK) {
        return;
    }

    llm::Manager::GetInstance().AddNewMcp(dlg.GetData());
    llm::Manager::GetInstance().ReloadConfig(std::nullopt, false);
}
