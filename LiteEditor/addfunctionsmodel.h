...
#ifndef __AddFunctionsModel_GUARD__H__
#define __AddFunctionsModel_GUARD__H__
 ...
 * To convert between a wxDataViewItem to a AddFunctionsModel_Item class, simply use:
 * @code
 * AddFunctionsModel_Item* itemData = reinterpret_cast<AddFunctionsModel_Item*>( item.GetID() );
 * if ( itemData ) {
 ...
 * @endcode
 */
class AddFunctionsModel_Item
{
protected:
    wxVector<wxVariant>        m_data;
    AddFunctionsModel_Item*           m_parent;
    wxVector<AddFunctionsModel_Item*> m_children;
    bool                       m_isContainer;
    wxClientData*              m_clientData;

public:
    AddFunctionsModel_Item()
        : m_parent(NULL)
        , m_isContainer(false)
        , m_clientData(NULL)
    {}
    virtual ~AddFunctionsModel_Item() {
        if ( m_clientData ) {
            delete m_clientData;
            m_clientData = NULL;
        }
        ...
        wxVector<AddFunctionsModel_Item*> tmpChildren = m_children;
     ...
     * @param child
     */
    void DeleteChild(AddFunctionsModel_Item* child) {
        wxVector<AddFunctionsModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
     ...
     * @param child
     */
    void RemoveChild(AddFunctionsModel_Item* child) {
        wxVector<AddFunctionsModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
        if ( iter != m_children.end() ) {
            m_children.erase(iter);
        }
     ...
     */
    void AddChild(AddFunctionsModel_Item* child) {
    ...
    // Setters / Getters
    void SetChildren(const wxVector<AddFunctionsModel_Item*>& children) {
        this->m_children = children;
    }
    void SetData(const wxVector<wxVariant>& data) {
        this->m_data = data;
    }
    void SetParent(AddFunctionsModel_Item* parent) {
        this->m_parent = parent;
    }
    const wxVector<AddFunctionsModel_Item*>& GetChildren() const {
        return m_children;
    }
    wxVector<AddFunctionsModel_Item*>& GetChildren() {
        return m_children;
    }
    ...
    AddFunctionsModel_Item* GetParent() {
        return m_parent;
    }
...
class AddFunctionsModel : public wxDataViewModel
{
protected:
    wxVector<AddFunctionsModel_Item*> m_data;
    unsigned int m_colCount;

public:
    AddFunctionsModel();
    virtual ~AddFunctionsModel();
    ...
    void SetData(const wxVector<AddFunctionsModel_Item*> data) {
        this->m_data = data;
    }
    unsigned int GetColCount() const {
        return m_colCount;
    }
    const wxVector<AddFunctionsModel_Item*>& GetData() const {
        return m_data;
    }
    wxVector<AddFunctionsModel_Item*>& GetData() {
        return m_data;
    }
...
#endif // __AddFunctionsModel_GUARD__H__
