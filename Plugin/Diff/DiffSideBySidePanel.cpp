...
{
    if (!left.Exists()) {
        ::wxMessageBox(wxString() << _("Left Side File:\n") << left.GetFullPath() << _(" does not exist!"),
                       "CodeLite",
                       wxICON_ERROR | wxCENTER | wxOK);
        return;
    }
...
void DiffSideBySidePanel::OnPaneloverviewEraseBackground(wxEraseEvent& event)
{
    ...
    if (m_stcLeft->LinesOnScreen() < lines) {
        ...
        if (m_config.IsSplitHorizontal()) {
            dc.DrawRectangle(
                0, yOffset + topVisibleLine * (ht - yOffset) / lines, x1, wxMax(depth * (ht - yOffset) / lines, 1));
        } else {
...
void DiffSideBySidePanel::OnCopyAllMenu(wxCommandEvent& event)
{
    ...
    menu.Bind(
        wxEVT_MENU, &DiffSideBySidePanel::OnCopyFileLeftToRight, this, XRCID("ID_DIFF_TOOL_COPY_ALL_LEFT_TO_RIGHT"));
    menu.Bind(
        wxEVT_MENU, &DiffSideBySidePanel::OnCopyFileFromRight, this, XRCID("ID_DIFF_TOOL_COPY_ALL_RIGHT_TO_LEFT"));
...
void DiffSideBySidePanel::OnPreferences(wxCommandEvent& event)
{
    ...
    menu.Bind(
        wxEVT_MENU, &DiffSideBySidePanel::OnIgnoreWhitespaceClicked, this, XRCID("ID_DIFF_TOOL_IGNORE_WHITESPACE"));
    menu.Bind(wxEVT_MENU, &DiffSideBySidePanel::OnShowLinenosClicked, this, XRCID("ID_DIFF_TOOL_SHOW_LINENUMBERS"));
    menu.Bind(
        wxEVT_MENU, &DiffSideBySidePanel::OnShowOverviewBarClicked, this, XRCID("ID_DIFF_TOOL_SHOW_OVERVIEW_BAR"));
