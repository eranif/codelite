    ...
    , m_notify(true)
{
    ...
    // Connect events
    Connect(
        wxEVT_COMMAND_AUINOTEBOOK_PAGE_CHANGED, wxAuiNotebookEventHandler(Notebook::OnInternalPageChanged), NULL, this);
    Connect(wxEVT_COMMAND_AUINOTEBOOK_PAGE_CHANGING,
            wxAuiNotebookEventHandler(Notebook::OnInternalPageChanging),
            NULL,
            this);
    Connect(
        wxEVT_COMMAND_AUINOTEBOOK_PAGE_CLOSED, wxAuiNotebookEventHandler(Notebook::OnInternalPageClosed), NULL, this);
    Connect(
        wxEVT_COMMAND_AUINOTEBOOK_PAGE_CLOSE, wxAuiNotebookEventHandler(Notebook::OnInternalPageClosing), NULL, this);
    ...
    m_mgr.SetFlags(wxAUI_MGR_ALLOW_FLOATING | wxAUI_MGR_ALLOW_ACTIVE_PANE | wxAUI_MGR_TRANSPARENT_DRAG |
...
Notebook::~Notebook()
{
    Disconnect(
        wxEVT_COMMAND_AUINOTEBOOK_PAGE_CHANGED, wxAuiNotebookEventHandler(Notebook::OnInternalPageChanged), NULL, this);
    Disconnect(wxEVT_COMMAND_AUINOTEBOOK_PAGE_CHANGING,
               wxAuiNotebookEventHandler(Notebook::OnInternalPageChanging),
               NULL,
               this);
    Disconnect(
    ...
    Disconnect(
        wxEVT_COMMAND_AUINOTEBOOK_TAB_RIGHT_DOWN, wxAuiNotebookEventHandler(Notebook::OnTabRightDown), NULL, this);
...
#endif
    Disconnect(
        SHOW_POPUP_MENU, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(Notebook::OnInternalMenu), NULL, this);
}
...
{
    if(page >= GetPageCount()) return NULL;
...
{
    if(page >= GetPageCount()) return false;
    ...
    wxWindow* pageToMove = GetPreviousSelection();
    if(pageToMove && (win != pageToMove)) {
        size_t pageIndex = GetPageIndex(pageToMove);
        if(pageIndex != Notebook::npos) {
            SetSelection(pageIndex, true);
        }
    }

    // Now delete this page
...
{
    if(page >= GetPageCount()) return wxT("");
...
const wxArrayPtrVoid& Notebook::GetHistory() const { return m_history; }

void Notebook::SetRightClickMenu(wxMenu* menu) { m_contextMenu = menu; }
...
void Notebook::GetEditorsInOrder(std::vector<wxWindow*>& editors)
{
    ...
    // wxAuiNotebook stores its wxAuiTabCtrls in an internal class wxTabFrame, defined in the cpp file, and so
    // unavailable
...
{
    if(index >= GetPageCount()) return false;
...
void Notebook::OnFocus(wxFocusEvent& e) { e.Skip(); }
