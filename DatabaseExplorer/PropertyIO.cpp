...
wxXmlNode*
xsPropertyIO::AddPropertyNode(wxXmlNode* parent, const wxString& name, const wxString& value, wxXmlNodeType type)
{
    if (parent) {
...
wxString xsStringPropIO::ToString(const wxString& value) { return value; }

wxString xsStringPropIO::FromString(const wxString& value) { return value; }
...
wxString xsCharPropIO::ToString(const wxChar& value) { return wxString::Format(wxT("%c"), value); }

wxChar xsCharPropIO::FromString(const wxString& value) { return value.GetChar(0); }
...
wxString xsLongPropIO::ToString(const long& value) { return wxString::Format(wxT("%ld"), value); }
...
wxString xsIntPropIO::ToString(const int& value) { return wxString::Format(wxT("%d"), value); }
...
wxString xsBoolPropIO::ToString(const bool& value) { return wxString::Format(wxT("%d"), value); }
...
wxString xsPointPropIO::ToString(const wxPoint& value) { return wxString::Format(wxT("%d,%d"), value.x, value.y); }
...
wxString xsSizePropIO::ToString(const wxSize& value) { return wxString::Format(wxT("%d,%d"), value.x, value.y); }
...
wxString xsRealPointPropIO::ToString(const wxRealPoint& value)
{
    return wxString::Format(
        wxT("%s,%s"), xsDoublePropIO::ToString(value.x).c_str(), xsDoublePropIO::ToString(value.y).c_str());
}
...
wxColour xsColourPropIO::FromString(const wxString& value)
{
    ...
    if (!value.IsEmpty()) {
        if (wxSscanf(value, wxT("%d,%d,%d,%d"), &nRed, &nGreen, &nBlue, &nAlpha) == 3)
            nAlpha = 255;
    }
...
{
    return wxString::Format(
        wxT("%s %d %d"), xsColourPropIO::ToString(value.GetColour()).c_str(), value.GetWidth(), value.GetStyle());
}
...
wxString xsFontPropIO::ToString(const wxFont& value) { return value.GetNativeFontInfoUserDesc(); }
...
void xsArrayRealPointPropIO::Read(xsProperty* property, wxXmlNode* source)
{
    ...
    while (listNode) {
        if (listNode->GetName() == wxT("item")) {
            ((RealPointArray*)property->m_pSourceVariable)
                ->Add(xsRealPointPropIO::FromString(listNode->GetNodeContent()));
        }
...
#else
            xsSerializable* object =
                (xsSerializable*)wxCreateDynamicObject(listNode->GetAttribute(wxT("type"), wxT("")));
#endif
            if (object) {
...
#if wxVERSION_NUMBER < 2900
        *(xsSerializable**)(property->m_pSourceVariable) =
            (xsSerializable*)wxCreateDynamicObject(objectNode->GetPropVal(wxT("type"), wxT("")));
#else
        *(xsSerializable**)(property->m_pSourceVariable) =
            (xsSerializable*)wxCreateDynamicObject(objectNode->GetAttribute(wxT("type"), wxT("")));
#endif
...
void xsMapStringPropIO::Read(xsProperty* property, wxXmlNode* source)
{
    ...
    while (listNode) {
        if (listNode->GetName() == wxT("item")) {
#if wxVERSION_NUMBER < 2900
            (*(StringMap*)property->m_pSourceVariable)[listNode->GetPropVal(wxT("key"), wxT("undef_key"))] =
                listNode->GetNodeContent();
#else
            (*(StringMap*)property->m_pSourceVariable)[listNode->GetAttribute(wxT("key"), wxT("undef_key"))] =
                listNode->GetNodeContent();
#endif
        }
