...
wxArrayString ParseBlame(wxStyledTextCtrl* stc, wxArrayString& blameArr, size_t& n) // Helper function. 'n' indexes any previous 'author ' field + 1
{   
    ...
    // Most fields start with a descriptive string e.g. 'author', 'filename', but the hash and the code-line itself don't.
    ...
    wxCHECK_MSG(hash.Len() > 39, arr, "What should have been the 'commit-hash' field is too short"); // Something went seriously wrong, or the output is very strange. Skip.
    hash = hash.Left(8);
    ...
    static int cw = stc->GetCharWidth();
    static int max_date_extent = cw * DATE_WIDTH;
    static int spacewidth = stc->GetTextExtent(" ").x;
    static int inbetweenwidth = (cw + spacewidth) / 2; // If we need to truncate an author-name, try with this before the more drastic cw
    static int marginwidth = stc->GetMarginWidth(TEXT_MARGIN_ID);
...
GitBlameDlg::GitBlameDlg(wxWindow* parent, GitPlugin* plugin)
    : GitBlameDlgBase(parent), m_plugin(plugin), m_displayLog(NULL)
{
    ...
    // Configure the text margin. It needs to be 24 chars wide for the date & hash, the max author-width, plus 2 spaces between
...
void GitBlameDlg::SetBlame(const wxString& blame, const wxString& args)
{
    ...
    size_t numlen = wxString::Format("%i", (int)count).length(); //How many digits must we allow room for in the number margin?
    ...
    m_stcBlame->SetReadOnly(false); // In case we're re-entering, ensure we're r/w. For a wxSTC 'readonly' seems to mean can't append text programatically either
    m_stcBlame->ClearAll();
...
void GitBlameDlg::OnSTCDwellStart(wxStyledTextEvent& event)
{
    ...
    if(event.GetX() > 0 && event.GetX() < m_stcBlame->GetMarginWidth(0)) {// It seems that we can get spurious events with x == 0
...
void GitBlameDlg::OnStcblameLeftDclick(wxMouseEvent& event)
{
    ...
    if (!commit.empty() && commit != "00000000" /*Not yet committed*/ && !filepath.empty()) {
        ...
        if (newBlame.empty()) {
            return; // This commit is parentless (it's probably the initial commit) so trying to access commit^ will fail
        }
...
void GitBlameDlg::OnBackUpdateUI(wxUpdateUIEvent& event)
{
    event.Enable(m_commitStore.CanGoBack());
}
...
void GitBlameDlg::OnForwardUpdateUI(wxUpdateUIEvent& event)
{
    event.Enable(m_commitStore.CanGoForward());
}
...
void LogPopupTransientWindow::SetText(const wxString& text)
{
    m_staticText->SetLabel(text);
}

void LogPopupTransientWindow::OnLeaveWindow(wxMouseEvent& WXUNUSED(event))
{
    Dismiss();
}
