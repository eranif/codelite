...
void Manager::CreateProject(ProjectData& data, const wxString& workspaceFolder)
{
    ...
    wxString errMsg;
    bool res = clCxxWorkspaceST::Get()->CreateProject(data.m_name,
                                                      data.m_path,
...
{
    // make the output pane visible
    const wxArrayString dbgPanes = StdToWX::ToArrayString({wxT("Debugger"),
...
void Manager::DebuggerUpdate(const DebuggerEventData& event)
{
    ...
    case DBG_UR_LISTCHILDREN: {
        ...
        } else if (event.m_userReason == QUERY_LOCAL_CHILDREN || event.m_userReason == LIST_LOCAL_CHILDREN ||
...
bool Manager::IsShutdownInProgress() { return ::IsShutdownInProgress(); }
void Manager::SetShutdownInProgress() { ::SetShutdownInProgress(); }
