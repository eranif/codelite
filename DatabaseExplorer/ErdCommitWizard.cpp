...
ErdCommitWizard::ErdCommitWizard(wxWindow* parent, xsSerializable* pConnections, const wxString& createScript)
    : wxWizard(parent, wxID_ANY, _T("ERD commit wizard"), wxBitmap(wiztest_xpm))
{
...
wxWizardPageSimple* ErdCommitWizard::GetFirstPage() { return m_pFirstPage; }
    ...
    : wxWizardPageSimple(parent)
{
    ...
    mainSizer->Add(new wxStaticText(
                       this, wxID_ANY,
                       wxString(_("Wizard for creating db structure\n")) + wxString(_("on ERD diagram base.\n\n")) +
...
DatabasePage::DatabasePage(ErdCommitWizard* parent, xsSerializable* pConnections)
    : wxWizardPageSimple(parent)
{
    ...
    while(connectionNode) {
        ...
        if(pDbCon) {
            wxTreeItemId rootID = m_treeDatabases->AppendItem(
                totalRootID, wxString::Format(_("Databases (%s)"), pDbCon->GetServerName().c_str()), -1, -1,
                new DbItem(pDbCon));
            ...
            while(dbNode) {
                ...
                if(pDatabase) {
                    // wxTreeItemId dbID = m_treeDatabases->AppendItem(rootID,pDatabase->getName(),-1,-1, new
                    // DbItem(pDatabase,NULL));//new DbDatabase(db->getName()));
                    m_treeDatabases->AppendItem(rootID, pDatabase->GetName(), -1, -1,
                                                new DbItem(pDatabase)); // new DbDatabase(db->getName()));
                }
...
BackupPage::BackupPage(ErdCommitWizard* parent)
    : wxWizardPageSimple(parent)
{
    m_pParentWizard = parent;
    backuped = false;
    ...
    m_pFileData = new wxFilePickerCtrl(this, wxID_ANY, _("data.sql"), _("Select file"), wxT("SQL file *.sql|*.sql"),
    ...
    m_pFileStructure = new wxFilePickerCtrl(this, wxID_ANY, _("structure.sql"), _("Select file"),
                                            wxT("SQL file *.sql|*.sql"), wxDefaultPosition, wxDefaultSize,
                                            wxFLP_USE_TEXTCTRL | wxFLP_SAVE | wxFLP_OVERWRITE_PROMPT);
...
bool BackupPage::TransferDataFromWindow()
{
    if(!backuped) {
        wxMessageDialog dlg(this,
                            _("Backing up database data before changing the structure is really good idea. Do you want "
                              "to continue without doing so?"),
                            _("Backup"), wxYES_NO);
...
void BackupPage::OnBtnBackupClick(wxCommandEvent& event)
{
    ...
    DumpClass* pDump = new DumpClass(m_pParentWizard->GetSelectedDatabase()->GetDbAdapter(),
                                     m_pParentWizard->GetSelectedDatabase(), m_pFileData->GetPath());
    dumpResult = pDump->DumpData();
...
void BackupPage::OnBtnBackupUI(wxUpdateUIEvent& event)
{
    if(m_pCheckStructure->IsChecked()) {
        ...
        event.Enable(!m_pFileData->GetPath().IsEmpty());
}
...
WriteStructurePage::WriteStructurePage(ErdCommitWizard* parent)
    : wxWizardPageSimple(parent)
{
    m_pParentWizard = parent;
    ...
    m_btnWrite->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(WriteStructurePage::OnBtnWriteClick), NULL,
                        this);
    m_btnShowSql->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(WriteStructurePage::OnBtnShowSqlClick),
                          NULL, this);
}
...
void WriteStructurePage::OnBtnWriteClick(wxCommandEvent& event)
{
    ...
    try {
        pDbLayer = m_pParentWizard->GetSelectedDatabase()->GetDbAdapter()->GetDatabaseLayer(
            m_pParentWizard->GetSelectedDatabase()->GetName());
        if(pDbLayer) {
            pDbLayer->BeginTransaction();

            wxString useSql = m_pParentWizard->GetSelectedDatabase()->GetDbAdapter()->GetUseDb(
                m_pParentWizard->GetSelectedDatabase()->GetName());
            if(!useSql.IsEmpty())
                pDbLayer->RunQuery(
                    wxString::Format(wxT("USE %s"), m_pParentWizard->GetSelectedDatabase()->GetName().c_str()));
...
{
    if(!commited) {
        wxMessageDialog dlg(this, _("Would you like to continue without writing the database structure?"),
                            _("Question"), wxYES_NO);
        if(dlg.ShowModal() == wxID_YES) {
            return true;
        } else
...
LastPage::LastPage(wxWizard* parent)
    : wxWizardPageSimple(parent)
{
    wxBoxSizer* mainSizer = new wxBoxSizer(wxVERTICAL);
    mainSizer->Add(new wxStaticText(this, wxID_ANY, _("Writing structure ended.\n")), 0, wxALL, 5);
    SetSizer(mainSizer);
    mainSizer->Fit(this);
}
...
RestorePage::RestorePage(ErdCommitWizard* parent)
    : wxWizardPageSimple(parent)
{
    m_pParentWizard = parent;
    m_mainSizer = new wxFlexGridSizer(1, 0, 0);
    ...
    m_restoreFile =
        new wxFilePickerCtrl(this, wxID_ANY, wxT(""), _("Select file"), wxT("SQL file *.sql|*.sql"), wxDefaultPosition,
    ...
    m_btnRestore->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(RestorePage::OnBtnRestoreClick), NULL,
                          this);
}
...
{
    if(m_restoreFile->GetPath().IsEmpty())
        return;
...
void RestorePage::OnBtnRestoreUI(wxUpdateUIEvent& event) { event.Enable(!m_restoreFile->GetPath().IsEmpty()); }
