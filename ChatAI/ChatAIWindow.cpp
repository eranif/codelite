...
void ChatAIWindow::OnSendUI(wxUpdateUIEvent& event) { event.Enable(m_state == llm::ChatState::kReady); }
...
{
    wxBusyCursor bc{};
    wxArrayString models = llm::Manager::GetInstance().LoadModels();
    m_activeModel->Clear();
...
void ChatAIWindow::OnStopUI(wxUpdateUIEvent& event) { event.Enable(m_state != ChatState::kReady); }
