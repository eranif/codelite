...
struct _SAscendingSortArgs {
    bool operator()(const PHPFuncArg& a, const PHPFuncArg& b) { return (b.getIndex() > a.getIndex()); }
};
...
void PHPEntry::save(wxSQLite3Database* db, long parentDbId)
{
    try {
        wxSQLite3Statement st =
            db->PrepareStatement("REPLACE INTO ENTRY_TABLE (ID, PARENT_ID, NAME, KIND, TYPE, TYPE_SCOPE, INHERITS, "
                                 "USING_TRAITS, IMPLEMENTS, DOC_COMMENT, ATTRIBUTES, DEFAULT_VALUE, NAMESPACE, "
                                 "LINE_NUMBER, FILE_NAME) VALUES ( NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
{
    bool b = type.find_first_not_of(wxT("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_\\->$")) ==
             wxString::npos;
...
void PHPEntry::addArgument(const PHPFuncArg& arg) { this->arguments.push_back(arg); }
...
void PHPEntry::sortArgs() { std::sort(arguments.begin(), arguments.end(), _SAscendingSortArgs()); }
...
bool PHPEntry::loadByNameAndNS(wxSQLite3Database* db,
                               const wxString& name,
                               const wxString& ns,
                               PHPEntry& cls,
                               PHPEntryKind kind,
...
bool PHPEntry::loadByParentIdAndName(wxSQLite3Database* db,
                                     long parentDbId,
                                     const wxString& name,
                                     PHPEntryKind kind,
                                     PHPEntry& entry,
...
void PHPEntry::GetParents(wxSQLite3Database* db,
                          const wxString& name,
                          const wxString& nameSpace,
                          PHPEntry::LongVector_t& ids,
                          PHPEntry::StringSet_t& scannedParents)
{
    ...
    PHPEntry cls;
    if(!PHPEntry::loadByNameAndNS(db, classname, name_space, cls)) return;
                          ...
                          PHPEntry::StringSet_t& scannedParents)
{
    ...
    PHPEntry cls;
    if(!PHPEntry::loadById(db, dbId, cls)) return;
    GetParents(db, cls, ids, scannedParents);
}

PHPEntry::Vector_t PHPEntry::loadByParentIdAndName(wxSQLite3Database* db,
                                                   long parentDbId,
                                                   const wxString& name,
                                                   bool partialMatch,
                                                   bool includeChildren)
    ...
    try {
        ...
        if(partialMatch) {
            ...
            if(!escapedName.IsEmpty()) {
                sql << "select * from ENTRY_TABLE where " << DoMakeParentIdSql(db, parentDbId) << " AND NAME like '"
                    << escapedName << "%%' ESCAPE '^' ORDER BY KIND DESC";
                DoAddLimitClause(sql);
...
PHPEntry::Vector_t
PHPEntry::loadByName(wxSQLite3Database* db, const wxString& filter, bool partialMatch, bool includeChildren)
{
    PHPEntry::Vector_t v;
    try {
        wxString sql;
...
PHPEntry::Vector_t PHPEntry::FindSuperGloabls(wxSQLite3Database* db, const wxString& name)
{
    ...
    try {
        wxString sql;
        wxString escapedName = EscapeString(name);
        sql << "select * from ENTRY_TABLE where NAME like '" << escapedName
            << "%%' ESCAPE '^' AND FILE_NAME LIKE '%%SuperGlobals.php'";
        DoAddLimitClause(sql);
        wxSQLite3Statement st = db->PrepareStatement(sql);
        wxSQLite3ResultSet res = st.ExecuteQuery();
        while(res.NextRow()) {
...
PHPEntry::Vector_t PHPEntry::FindGloablFunctions(wxSQLite3Database* db, const wxString& name)
{
    ...
    try {
        wxString sql;
        wxString escapedName = EscapeString(name);
        sql << "select * from ENTRY_TABLE where NAME like '" << escapedName << "%%' ESCAPE '^' AND KIND IN("
            << (int)PHP_Kind_Function << ") ";
        DoAddLimitClause(sql);
        wxSQLite3Statement st = db->PrepareStatement(sql);
        wxSQLite3ResultSet res = st.ExecuteQuery();
        while(res.NextRow()) {
...
PHPEntry::Vector_t
PHPEntry::LoadNamespaceEntries(wxSQLite3Database* db, const wxString& ns, const wxString& partname, size_t flags)
{
    PHPEntry::Vector_t arr;
    try {
        wxString sql;
        ...
        sql << "select * from ENTRY_TABLE where NAMESPACE = ? AND KIND IN (" << (int)PHP_Kind_Class << ", "
            << (int)PHP_Kind_Namespace << ") ";
...
void PHPEntry::DoAddLimitClause(wxString& sql) { sql << " LIMIT " << PHPStorage::Instance()->GetLimit(); }

void PHPEntry::addInterface(const wxString& iface) { implements.Add(iface); }

void PHPEntry::setInherits(const wxString& inherits) { this->inherits = inherits; }
