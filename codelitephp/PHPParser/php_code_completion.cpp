...
wxString PHPCodeCompletion::DoGetExpression(IEditor* editor, int pos)
{
    ...
    while(cont && depth >= 0) {
        ...
        if(style == wxSTC_H_COMMENT || style == wxSTC_H_XCCOMMENT || style == wxSTC_H_SGML_COMMENT ||
...
PHPEntry::Vector_t PHPCodeCompletion::DoSuggestGlobals(const wxString& partname, const wxString& ns)
{
    ...
    PHPEntry::Vector_t dbResutls =
        PHPStorage::Instance()->FetchNamespaceEntries(ns, partname, PHPStorage::FetchFlags_PartMatch);
    std::sort(dbResutls.begin(), dbResutls.end(), PHPEntry::LowerThan());

    // Always load the global functions
    PHPEntry::Vector_t dbResutlsFunctions =
        PHPStorage::Instance()->FindGloablFunctions(partname, PHPStorage::FetchFlags_PartMatch);
    std::sort(dbResutlsFunctions.begin(), dbResutlsFunctions.end(), PHPEntry::LowerThan());

    // Suggest the super globals
    PHPEntry::Vector_t dbSGResutlsFunctions =
        PHPStorage::Instance()->FindSuperGloabls(partname, PHPStorage::FetchFlags_PartMatch);
    std::sort(dbSGResutlsFunctions.begin(), dbSGResutlsFunctions.end(), PHPEntry::LowerThan());

    // Merge the results
    std::merge(dbResutls.begin(),
               dbResutls.end(),
               dbResutlsFunctions.begin(),
               dbResutlsFunctions.end(),
               std::back_inserter(v));
    std::merge(dbResutls.begin(),
               dbResutls.end(),
               dbSGResutlsFunctions.begin(),
               dbSGResutlsFunctions.end(),
               std::back_inserter(v));
...
void PHPCodeCompletion::DoShowCompletionBox(const std::vector<PHPEntry>& entries,
...
TagEntryPtr PHPCodeCompletion::DoPHPEntryToTagEntry(const PHPEntry& entry)
{
    ...
    case PHP_Kind_Function: {
        ...
        for(size_t i = 0; i < entry.getArguments().size(); i++) {
            ...
            if(arg.getAttributes() & PHP_Attr_Reference) signature << wxT("&");
...
// bool PHPCodeCompletion::DoExpandFunctionReturnValue(PHPEntry* entry, std::map<wxString, PHPEntry>& locals, bool
// &stop)
//{
//    stop = true;
...
//        if(PHPStorage::Instance()->FindFunction(entry->getParent(), currentNameSpace, entry->getName(),
//        PHPStorage::FetchFlags_ExactMatch, *entry))
//            success = true;
//    }
//
...
//            if(iter != locals.end()) {
//                DoCopyEntryProperties(iter->second, *entry);
//                stop = (entryType == entry->getType()); // Avoid recursion by making sure that the new type is
//                different from the new expanded type
//                return true;
//            }
//
...
//            // Baz()->
//            // </code>
//            if(PHPStorage::Instance()->FindFunction(entry->getTypeScope(), currentNameSpace, entryType,
//            PHPStorage::FetchFlags_ExactMatch, *entry)) {
//                stop = (entryType == entry->getType()); // Avoid recursion by making sure that the new type is
//                different from the new expanded type
//                return true;
    ...
    while(cur) {
        ...
        if(cur->isHead() && cur->isStartsWithNSOperator()) {
            ...
            if(currentScope.isNamespace()) {
                ...
                if(!PHPStorage::Instance()
                        ->FindByFullPath(currentScope.getContainerPath() + "\\" + cur->getName(), match))
                    return false;

            } else if(!PHPStorage::Instance()
                           ->FindEntryByNameAndParentId(currentScope.getDbId(), cur->getName(), match)) {
                if(!PHPStorage::Instance()
                        ->FindEntryByNameAndParentId(currentScope.getParentDbId(), cur->getName(), match)) {
                    // Last try: search for the global namespace for function only
                    if(!PHPStorage::Instance()
                            ->FindEntryByNameAndParentId(-1, cur->getName(), PHP_Kind_Function, match)) {
                        return false;
                    }
                }
            }
...
{
    if(!PHPWorkspace::Get()->IsOpen()) return PHPEntry();

    pos = editor->GetSTC()->WordEndPosition(pos, true);
    DoInitialize(editor, pos);

    // Second parse is needed to get the locals at the current position
    if(!PHPParser.parseString(true, editor->GetTextRange(0, pos), PHPGlobals::Type_Normal)) return PHPEntry();

    // Extract the expression
    wxString expr = DoGetExpression(editor, pos);
    if(!PHPParser.parseString(true, PHP_PREFIX_WITH_SPACE + expr, PHPGlobals::Type_Expr)) return PHPEntry();
...
bool PHPCodeCompletion::TestSuggestMembers(const wxString& fileContent,
...
void PHPCodeCompletion::DoGetMembersOf(const PHPEntry& entry,
                                       const wxString& partname,
                                       size_t filter_flags,
                                       PHPEntry::Vector_t& res)
{
    ...
    if(entry.isNamespace()) {
        ...
        if(filter_flags & kCC_SELF) {
            ...
            for(size_t i = 0; i < parentsIds.size(); ++i) {
                if(!partname.IsEmpty()) {
                    tmpRes =
                        PHPStorage::Instance()->FindEntriesByParentIdAndName(parentsIds.at(i), partname, fetchFlags);
                } else {
...
PHPEntry PHPCodeCompletion::GetClassNearPosition(IEditor* editor, int pos)
{
    ...
    if(PHPParser.getCurrentClass()) {
        ...
        if(entry->isClass()) {
            PHPEntry cls;
            if(PHPStorage::Instance()
                   ->FindClass(entry->getName(), entry->getNameSpace(), PHPStorage::FetchFlags_ExactMatch, cls)) {
                return cls;
            }
        }
    }
