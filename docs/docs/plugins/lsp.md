...
CodeLite offers a small utility: `/usr/bin/codelite-cc` which does the following:

- Wraps your compiler (`gcc`, `clang` etc)
- Captures the compile line generated by your `make`, write it into a file and executes it
- You can then run it again (once the build process is over) to generate a `compile_commands.json` file
...
!!! Note
    ...
    In the below example, we rely on the fact that most `Makefile` files honor the standard
...
export CL_COMPILATION_DB=/tmp/out.txt
...
# `--finalize` will work on the file pointed by `CL_COMPILATION_DB`
