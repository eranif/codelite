...
enum { KindFileNode = 0, KindSingleEntry };
...
public:
    ...
    // Setters
    void SetFile(const wxString& file) { this->m_file = file.c_str(); }
    void SetKind(int kind) { this->m_kind = kind; }
    void SetLine(int line) { this->m_line = line; }
    void SetPattern(const wxString& pattern) { this->m_pattern = pattern.c_str(); }
