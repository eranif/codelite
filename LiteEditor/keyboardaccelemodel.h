...
#ifndef __KeyboardAcceleModel_GUARD__H__
#define __KeyboardAcceleModel_GUARD__H__
 ...
 * To convert between a wxDataViewItem to a KeyboardAcceleModel_Item class, simply use:
 * @code
 * KeyboardAcceleModel_Item* itemData = reinterpret_cast<KeyboardAcceleModel_Item*>( item.GetID() );
 * if ( itemData ) {
 ...
 * @endcode
 */
class KeyboardAcceleModel_Item
{
protected:
    wxVector<wxVariant>        m_data;
    KeyboardAcceleModel_Item*           m_parent;
    wxVector<KeyboardAcceleModel_Item*> m_children;
    bool                       m_isContainer;
    wxClientData*              m_clientData;

public:
    KeyboardAcceleModel_Item()
        : m_parent(NULL)
        , m_isContainer(false)
        , m_clientData(NULL)
    {}
    virtual ~KeyboardAcceleModel_Item() {
        if ( m_clientData ) {
            delete m_clientData;
            m_clientData = NULL;
        }
        ...
        wxVector<KeyboardAcceleModel_Item*> tmpChildren = m_children;
     ...
     * @param child
     */
    void DeleteChild(KeyboardAcceleModel_Item* child) {
        wxVector<KeyboardAcceleModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
     ...
     * @param child
     */
    void RemoveChild(KeyboardAcceleModel_Item* child) {
        wxVector<KeyboardAcceleModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
        if ( iter != m_children.end() ) {
            m_children.erase(iter);
        }
     ...
     */
    void AddChild(KeyboardAcceleModel_Item* child) {
    ...
    // Setters / Getters
    void SetChildren(const wxVector<KeyboardAcceleModel_Item*>& children) {
        this->m_children = children;
    }
    void SetData(const wxVector<wxVariant>& data) {
        this->m_data = data;
    }
    void SetParent(KeyboardAcceleModel_Item* parent) {
        this->m_parent = parent;
    }
    const wxVector<KeyboardAcceleModel_Item*>& GetChildren() const {
        return m_children;
    }
    wxVector<KeyboardAcceleModel_Item*>& GetChildren() {
        return m_children;
    }
    ...
    KeyboardAcceleModel_Item* GetParent() {
        return m_parent;
    }
...
class KeyboardAcceleModel : public wxDataViewModel
{
protected:
    wxVector<KeyboardAcceleModel_Item*> m_data;
    unsigned int m_colCount;

public:
    KeyboardAcceleModel();
    virtual ~KeyboardAcceleModel();
    ...
    void SetData(const wxVector<KeyboardAcceleModel_Item*> data) {
        this->m_data = data;
    }
    unsigned int GetColCount() const {
        return m_colCount;
    }
    const wxVector<KeyboardAcceleModel_Item*>& GetData() const {
        return m_data;
    }
    wxVector<KeyboardAcceleModel_Item*>& GetData() {
        return m_data;
    }
...
#endif // __KeyboardAcceleModel_GUARD__H__
