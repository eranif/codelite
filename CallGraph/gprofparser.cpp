    ...
    while (!gprof_output->Eof()) {
        ...
        if (readlinetext != wxT("")) {
            if (readlinetext == wxT("index % time    self  children    called     name")) {
                ...
                if (readlinetext.Contains(wxT(".")))
                    isdot = true;
                else
                    isdot = false;

                if (readlinetext.Contains(wxT("cycle")))
                    iscycle = true;
                else
                    iscycle = false;
                ...
                if ((readlinetext[0] == '[') && (readlinetext[(readlinetext.length()) - 1] == ']')) {
                    ...
                    if (iscycle && isplus) { // [3]     91.71    1.77        0.00    1+5    <cycle 1 as a whole> [3]
                        // warning
                        sscanf((const char*)readlinetext.mb_str(conv),
                               "[%d] %f %f %f %d %d <cycle %d as a whole> [%d]",
                               &line->index,
                               &line->time,
                               &line->self,
                               &line->children,
                        ...
                        wxSscanf(readlinesubtext.AfterFirst(wxT('<')).BeforeFirst(wxT('>')),
                                 wxT("cycle %d"),
                                 &line->cycleid);
                        ...
                        line->cycle = false;
                        line->recursive = true;
                    } else if (isspontaneous) { // || !readlinetext.Contains(wxT('('))) // [3]    100.0    0.00    0.03
                                                // %d - neni        main [3]
...
int GprofParser::GetSuggestedNodeThreshold()
{
    ...
    for (size_t i = 0; i < sortedCalls.GetCount() && totalCount < 100; ++i) {
        ...
        if (totalCount < 100 && sortedCalls[i] < minCallTime)
            minCallTime = sortedCalls[i];
    }

    if (minCallTime < 0)
        minCallTime = 0;
    else if (minCallTime > 100)
        minCallTime = 100;

    if (sortedCalls.GetCount() > 1 && totalCount >= 100)
        return minCallTime;
    else
        return -1;
}
