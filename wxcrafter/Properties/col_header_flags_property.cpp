...
ColHeaderFlagsProperty::ColHeaderFlagsProperty(const wxString& label,
                                               int initialValue,
                                               const wxString& tip,
                                               eColumnKind kind)
    : PropertyBase(tip)
...
wxString ColHeaderFlagsProperty::GetValue() const
{
    ...
    for (size_t i = 0; i < m_values.GetCount(); ++i) {
        if (m_initialValue & m_values.Item(i)) {
            str_value << m_names.Item(i) << "|";
        }
    }
    if (!str_value.IsEmpty()) {
        str_value.RemoveLast();
    }
    return str_value;
}
...
void ColHeaderFlagsProperty::SetValue(const wxString& value)
{
    ...
    for (size_t i = 0; i < selections.GetCount(); ++i) {
        ...
        if (index != wxNOT_FOUND) {
            m_initialValue |= m_values.Item(index);
        }
    }
}
