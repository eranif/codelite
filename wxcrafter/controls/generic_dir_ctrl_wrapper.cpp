    ...
    : wxcWidget(ID_WXGENERICDIRCTRL)
{
    ...
    RegisterEvent(wxT("wxEVT_COMMAND_TREE_BEGIN_DRAG"),
                  wxT("wxTreeEvent"),
    ...
    RegisterEvent(wxT("wxEVT_COMMAND_TREE_BEGIN_RDRAG"),
                  wxT("wxTreeEvent"),
                  _("The user has started dragging an item with the right mouse button.\nThe event handler must call "
                    "wxTreeEvent::Allow() for the drag operation to continue."));
    RegisterEvent(wxT("wxEVT_COMMAND_TREE_END_DRAG"),
                  wxT("wxTreeEvent"),
    ...
    RegisterEvent(wxT("wxEVT_COMMAND_TREE_END_LABEL_EDIT"),
                  wxT("wxTreeEvent"),
    ...
    RegisterEvent(wxT("wxEVT_COMMAND_TREE_ITEM_EXPANDING"),
                  wxT("wxTreeEvent"),
    ...
    RegisterEvent(wxT("wxEVT_COMMAND_TREE_ITEM_COLLAPSING"),
                  wxT("wxTreeEvent"),
    ...
    RegisterEvent(wxT("wxEVT_COMMAND_TREE_SEL_CHANGING"),
                  wxT("wxTreeEvent"),
...
wxString GenericDirCtrlWrapper::CppCtorCode() const
{
    ...
    if (filter.IsEmpty() == false) {
        code << GetName() << wxT("->SetFilter(") << wxCrafter::WXT(filter) << wxT(");\n");
    }
    ...
    if (-1 != index) {
        code << GetName() << wxT("->SetFilterIndex(") << index << wxT(");\n");
    }
...
void GenericDirCtrlWrapper::LoadPropertiesFromXRC(const wxXmlNode* node)
{
    ...
    if (propertynode) {
        SetPropertyString(PROP_DEFAULT_FOLDER, propertynode->GetNodeContent());
    }

    propertynode = XmlUtils::FindFirstByTagName(node, wxT("filter"));
    if (propertynode) {
        SetPropertyString(PROP_FILTER, propertynode->GetNodeContent());
    }

    propertynode = XmlUtils::FindFirstByTagName(node, wxT("defaultfilter"));
    if (propertynode) {
        SetPropertyString(PROP_DEFAULT_FILTER, propertynode->GetNodeContent());
    }
}
...
void GenericDirCtrlWrapper::LoadPropertiesFromwxFB(const wxXmlNode* node)
{
    ...
    if (propertynode) {
        ...
        if (prop) {
            prop->SetValue(propertynode->GetNodeContent());
        }
    }
}
