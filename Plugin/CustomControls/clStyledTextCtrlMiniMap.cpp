...
// Purpose:     Implementation of clStyledTextCtrlMiniMap.
...
int FindClosestEditFirstLine(int editNewInitial,
...
// ----------------------------------------------------------------------------
// clStyledTextCtrlMiniMap implementation
// ----------------------------------------------------------------------------

bool clStyledTextCtrlMiniMap::Create(
    wxWindow* parent, wxStyledTextCtrl* edit, wxWindowID id, const wxPoint& pos, const wxSize& size, long style)
{
    if (!wxStyledTextCtrl::Create(parent, id, pos, size, style))
        return false;
    ...
    Bind(wxEVT_STC_UPDATEUI, &clStyledTextCtrlMiniMap::OnMapUpdate, this);

    // Handle mouse events for dragging the visible zone indicator.
    Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent& event) { HandleMouseClick(event); });
    ...
    Bind(wxEVT_MOUSE_CAPTURE_LOST, [this](wxMouseCaptureLostEvent& WXUNUSED(event)) { DoStopDragging(); });

    // Draw our overlay over the map.
    Bind(wxEVT_PAINT, &clStyledTextCtrlMiniMap::OnMapPaint, this);

    return true;
}

clStyledTextCtrlMiniMap::~clStyledTextCtrlMiniMap()
{
    if (IsDragging())
        DoStopDragging();
...
void clStyledTextCtrlMiniMap::SetEdit(wxStyledTextCtrl* edit)
{
    if (edit == m_edit)
        return;
    ...
    if (m_edit) {
        ...
        m_edit->Unbind(wxEVT_STC_UPDATEUI, &clStyledTextCtrlMiniMap::OnEditUpdate, this);
        m_edit->Unbind(wxEVT_PAINT, &clStyledTextCtrlMiniMap::OnEditPaint, this);
        m_edit->Unbind(wxEVT_SIZE, &clStyledTextCtrlMiniMap::OnEditSize, this);
        m_edit->Unbind(wxEVT_DESTROY, &clStyledTextCtrlMiniMap::OnEditDestroy, this);
    }
    ...
    edit->Bind(wxEVT_STC_UPDATEUI, &clStyledTextCtrlMiniMap::OnEditUpdate, this);
    ...
    edit->Bind(wxEVT_SIZE, &clStyledTextCtrlMiniMap::OnEditSize, this);
    edit->Bind(wxEVT_PAINT, &clStyledTextCtrlMiniMap::OnEditPaint, this);

    // Stop using the editor when it is destroyed.
    edit->Bind(wxEVT_DESTROY, &clStyledTextCtrlMiniMap::OnEditDestroy, this);
    ...
    wxLogTrace(wxTRACE_STC_MAP, "Edit line height: %d, map: %d", m_edit->TextHeight(0), TextHeight(0));
}

void clStyledTextCtrlMiniMap::SetThumbColours(const wxColour& colNormal, const wxColour& colDragging)
{
    if (colNormal.IsOk())
...
bool clStyledTextCtrlMiniMap::UpdateLineInfo()
{
    LinesInfo lineInfo;
    ...
    lineInfo.mapMax = mapDisplay > lineInfo.mapVisible ? mapDisplay - lineInfo.mapVisible : 0;

    auto const editDisplay = m_edit->GetDisplayLineCount();
    lineInfo.editMax = editDisplay > lineInfo.editVisible ? editDisplay - lineInfo.editVisible : 0;
    ...
    lineInfo.thumbMax = VisibleFromDocLine(m_edit->DocLineFromVisible(lineInfo.editMax)) - lineInfo.mapMax;
...
// first visible line.
int clStyledTextCtrlMiniMap::GetMapFirstFromEditFirst(int editFirst) const
{
    auto const editDocFirst = m_edit->DocLineFromVisible(editFirst);
    auto const thumbPos = ScaleByRatio(editFirst, m_lines.thumbMax, m_lines.editMax);

    return VisibleFromDocLine(editDocFirst) - thumbPos;
}

// Scroll the editor to correspond to the current position in the map.
void clStyledTextCtrlMiniMap::SyncEditPosition()
{
    ...
    const int editNew = FindClosestEditFirstLine(
        ...
        [this](int editNew) { return GetMapFirstFromEditFirst(editNew); },
        mapFirst,
        m_lines.editMax);
...
void clStyledTextCtrlMiniMap::SyncMapPosition()
{
...
clStyledTextCtrlMiniMap::ThumbInfo clStyledTextCtrlMiniMap::GetThumbInfo() const
{
    ThumbInfo thumb;
...
void clStyledTextCtrlMiniMap::DrawVisibleZone(wxDC& dc)
{
...
void clStyledTextCtrlMiniMap::OnMapPaint(wxPaintEvent& event)
{
    if (!m_edit) {
        event.Skip();
        return;
...
void clStyledTextCtrlMiniMap::OnEditDestroy(wxWindowDestroyEvent& event)
{
    SetEdit(nullptr);
...
void clStyledTextCtrlMiniMap::OnEditSize(wxSizeEvent& event)
{
    wxLogTrace(
        wxTRACE_STC_MAP, "Edit resized to %dx%d, line info: %s", event.GetSize().x, event.GetSize().y, m_lines.Dump());
...
void clStyledTextCtrlMiniMap::OnEditPaint(wxPaintEvent& event)
{
    // The number of displayed lines may have changed due to
    // rewrapping, in which case we need to update the map.
    if (UpdateLineInfo()) {
        wxLogTrace(wxTRACE_STC_MAP, "Sync map from paint: %s", m_lines.Dump());
        SyncMapPosition();
    }
...
void clStyledTextCtrlMiniMap::OnEditUpdate(wxStyledTextEvent& event)
{
    ...
    if (event.GetUpdated() & wxSTC_UPDATE_V_SCROLL) {
        ...
        wxLogTrace(wxTRACE_STC_MAP, "Sync map from scroll: %d", editFirst);
...
void clStyledTextCtrlMiniMap::OnMapUpdate(wxStyledTextEvent& event)
{
    ...
    if (event.GetUpdated() & wxSTC_UPDATE_V_SCROLL) {
        ...
        wxLogTrace(wxTRACE_STC_MAP, "Sync edit from scroll: %d", mapFirst);

        SyncEditPosition();
    }
}

void clStyledTextCtrlMiniMap::HandleMouseClick(wxMouseEvent& event)
{
    if (!m_edit)
        return;
    ...
    wxLogTrace(wxTRACE_STC_MAP, "Clicked doc/editor line %d/%d", docLine, editLine);
    ...
    auto const editNew = GetValidLine(editLine - m_lines.editVisible / 2, m_lines.editMax);

    if (editNew != editFirst) {
        wxLogTrace(wxTRACE_STC_MAP, "Editor first %d -> %d", editFirst, editNew);
        SetEditFirstVisibleLine(editNew);
        SyncMapPosition();
...
void clStyledTextCtrlMiniMap::StartDragging(const wxPoint& pos)
{
    m_isDragging = true;
    ...
    wxLogTrace(wxTRACE_STC_MAP, "Dragging: start at offset=%d", m_dragOffset);
...
void clStyledTextCtrlMiniMap::DoDrag(wxPoint pos)
{
    if (!m_edit)
        return;
    ...
    const int editNew = FindClosestEditFirstLine(
        ...
        [this](int editNew) {
            ...
            return (MapVisibleFromEditVisible(editNew) + MapVisibleFromEditVisible(editNew + m_lines.editVisible)) / 2 -
...
void clStyledTextCtrlMiniMap::DoStopDragging()
{
    m_isDragging = false;
    m_dragOffset = 0;
...
wxString clStyledTextCtrlMiniMap::LinesInfo::Dump() const
{
    return wxString::Format(
        "map/edit visible: %d/%d, map/edit first line max: %d/%d", mapVisible, editVisible, mapMax, editMax);
}
#endif
