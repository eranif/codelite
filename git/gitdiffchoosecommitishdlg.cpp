    ...
    , m_selectedRadio2(3)
{
    ...
    m_process = CreateAsyncProcessCB(this, new GitDiffCmdHandler(m_choiceBranch1, m_choiceBranch2), command, IProcessCreateDefault, m_plugin->GetRepositoryDirectory());
...
void GitDiffChooseCommitishDlg::OnUpdateUIBranch1(wxUpdateUIEvent& event)
{
    event.Enable(m_radioBranch1->GetValue());
}
void GitDiffChooseCommitishDlg::OnUpdateUIBranch2(wxUpdateUIEvent& event)
{
    event.Enable(m_radioBranch2->GetValue());
}
void GitDiffChooseCommitishDlg::OnUpdateUICommit1(wxUpdateUIEvent& event)
{
    event.Enable(m_radioCommit1->GetValue());
}
void GitDiffChooseCommitishDlg::OnUpdateUICommit2(wxUpdateUIEvent& event)
{
    event.Enable(m_radioCommit2->GetValue());
}
...
void GitDiffChooseCommitishDlg::OnUpdateUITags1(wxUpdateUIEvent& event)
{
    event.Enable(m_radioTag1->GetValue());
}
void GitDiffChooseCommitishDlg::OnUpdateUITags2(wxUpdateUIEvent& event)
{
    event.Enable(m_radioTag2->GetValue());
}
...
{
    wxString text = (m_activeChoice1 == m_comboCommitish1) ?
        m_comboCommitish1->GetValue() : m_activeChoice1->GetStringSelection();
    if (text.StartsWith("* ")) {
...
{
    wxString text = (m_activeChoice2 == m_comboCommitish2) ?
        m_comboCommitish2->GetValue() : m_activeChoice2->GetStringSelection();
    if (text.StartsWith("* ")) {
        text = text.Mid(2); // Remove the 'active branch' marker
    }
