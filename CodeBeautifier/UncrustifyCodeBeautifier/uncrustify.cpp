...
int UncrustifyExecute(int argc, char *argv[], void (*onError)(const char *))
{	
   string     cfg_file;
   const char *parsed_file = NULL;
   const char *source_file = NULL;
   ...
   if (replace || no_backup)
   {
      if ((prefix != NULL) || (suffix != NULL))
      {
		 onError("Cannot use --replace with --prefix or --suffix");
	     return 66;
      }
      if ((source_file != NULL) || (output_file != NULL))
      {
		 onError("Cannot use --replace or --no-backup with -f or -o");
	     return 66;
      }
   }
   else
   ...
   {	   
      ...
      if (load_option_file(cpd.filename) < 0)
      {
		 onError("Unable to load the config file");
		 return 56;
      }
   }
   ...
   if (cfg_file.empty())
   {
	  onError("Specify the config file with '-c file' or set UNCRUSTIFY_CONFIG.");
	  return 58;
   }
   ...
   if ((source_list != NULL) || (p_arg != NULL))
   {
      if (source_file != NULL)
      {
	     onError("Cannot specify both the single file option and a multi-file option.");
	     return 67;		  
      }

      if (output_file != NULL)
      {
	     onError("Cannot specify -o with a multi-file option.");
	     return 68;
      }
   }
