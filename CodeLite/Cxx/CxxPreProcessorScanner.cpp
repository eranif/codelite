...
bool CxxPreProcessorScanner::CheckIf(const CxxPreProcessorToken::Map_t& table)
{
    ...
    while (m_scanner && ::LexerNext(m_scanner, token)) {
        ...
        case T_PP_GTEQ:
            cur = cur->SetNext(
                CxxPreProcessorExpression::kGreaterThanEqual, std::make_unique<CxxPreProcessorExpression>(0));
            break;
        case T_PP_LT:
        ...
        case T_PP_LTEQ:
            cur = cur->SetNext(
                CxxPreProcessorExpression::kLowerThanEqual, std::make_unique<CxxPreProcessorExpression>(0));
            break;
        default:
            break;
        }
